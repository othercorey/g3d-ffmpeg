...
void testFastPODTable() {
    typedef FastPODTable<int, int, HashTrait<int>, EqualsTrait<int>, true> TestTable;

    TestTable table;
    for (int i = 0; i < 100; ++i) {
        const int k = i * 10;
        table[k] = i;
        debugAssert(table.containsKey(k));
        debugAssert(table.size() == i + 1);
        debugAssert(table[k] == i);
    }
...
void measureNormalizationPerformance() {
    PRINT_SECTION("Performance: Normalization", "");
    Stopwatch stopwatch;
    chrono::nanoseconds raw, opt, overhead;
    int n = 1024 * 1024;
    ...
    chrono::nanoseconds r = raw;
    chrono::nanoseconds o = opt;
    chrono::nanoseconds h = overhead;

    PRINT_HEADER("Vector3");
    PRINT_NANO("direction()", "(ns)", (r - h) / (n*3.0));
    PRINT_NANO("fastDirection()", "(ns)", (o - h) / (n*3.0));
}
...
void measureRDPushPopPerformance(RenderDevice* rd) {
    PRINT_SECTION("Performance: RenderDevice", "Push and Pop State");
    int N = 500;
    rd->pushState();
    rd->popState();
    ...
    stopwatch.tock();
    chrono::nanoseconds pushPopTime = stopwatch.elapsedDuration();

    PRINT_MICRO("Push + Pop", "(us)", pushPopTime / (double)N);
}
...
void perfHashTrait() {
    PRINT_SECTION("Hash functions for Vector3", "");
    Stopwatch stopwatch;
    ...
    const Vector3 v(100, 32, 0.11f);
    {
        ...
        (void)h;
        stopwatch.tock();
        PRINT_MICRO("Vector3", "(us)", stopwatch.elapsedDuration());
    }
    {
        stopwatch.tick();
        size_t h = 0;
        ...
        (void)h;
        stopwatch.tock();
        PRINT_MICRO("Crypto::crc32", "(us)", stopwatch.elapsedDuration());
    }
    {
        stopwatch.tick();
        size_t h = 0;
        ...
        (void)h;
        stopwatch.tock();
        PRINT_MICRO("Crypto::md5", "(us)", stopwatch.elapsedDuration());
    }
    {
        stopwatch.tick();
        size_t h = 0;
        ...
        (void)h;
        stopwatch.tock();
        PRINT_MICRO("HashTrait", "(us)", stopwatch.elapsedDuration());
    }
}
