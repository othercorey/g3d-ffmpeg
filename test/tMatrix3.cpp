...
void perfMatrix3() {
    PRINT_SECTION("Performance: Matrix3", "");
    Stopwatch stopwatch;
    chrono::nanoseconds raw, opt, overhead, naive;
    ...
    PRINT_HEADER("Transpose Performance");
    PRINT_MICRO("transpose(A, C)", "(us/mul)", opt / (3 * n));
    PRINT_MICRO("C = A.transpose()", "(us/mul)", raw / (3 * n));
    // These measurements were originally compared to cycles
    //PRINT_TEXT("Outcome", (opt / (3 * n) < 400) ? " ok " : "FAIL", (raw / (3 * n) < 150) ? " ok " : "FAIL");
    ...
    
    {
        ...
        stopwatch.tock();
        naive = stopwatch.elapsedDuration();
    }

    raw -= overhead;
    opt -= overhead;

    PRINT_HEADER("Matrix-Matrix Multiplication");
    PRINT_MICRO("mul(A, B, C)", "(us/mul)", opt / (3 * n));
    PRINT_MICRO("C = A * B", "(us/mul)", raw / (3 * n));
    PRINT_MICRO("naive for-loops", "(us/mul)", naive / (3 * n));
    // These measurements were originally compared to cycles
    //PRINT_TEXT("Outcome", (opt / (3 * n) < 400) ? " ok " : "FAIL", (raw / (3 * n) < 150) ? " ok " : "FAIL");
}
