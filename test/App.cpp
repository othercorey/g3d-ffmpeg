...
static const String RENDER_TEST_DIRECTORY      = "RenderTest/";
...
static const Array<String> TEST_SCENE_LIST = { "G3D Simple Cornell Box", "G3D Sponza", "G3D Test Models" };
...
bool App::setupGoldStandardMode() const {
    // Assumptions abound
    return settings().screenCapture.outputDirectory.find("GoldStandard") != String::npos;
}
...
void App::saveAndPossiblyCompareTextureToGoldStandard(const String& name, const shared_ptr<Texture>& texture) {
    ...
    im->save(FilePath::concat(settings().screenCapture.outputDirectory, name + ".exr"));
    if (! setupGoldStandardMode()) {
...
void App::onGraphics3D(RenderDevice* rd, Array<shared_ptr<Surface> >& allSurfaces) {
    ...
    if (m_frameCount > 3) {
		...
		m_film->exposeAndRender(rd, activeCamera()->filmSettings(), m_framebuffer->texture(0), settings().hdrFramebuffer.trimBandThickness().x,
            settings().hdrFramebuffer.depthGuardBandThickness.x, resultTexture);
