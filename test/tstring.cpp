...
void teststring() {
    ...
    //testing greatest common prefix
    {
        ...
        testAssertM(greatestCommonPrefix("G3D:Cornell Box", "G3D-base/Cornell Box") == "", "String gcp() broken");
    }

    //prefix tree building tests
    {
        Array<String> hello = { "Hello G3D", "Hello World", "G3D-base/scene/demo", "G3D-base/scene/scene" };
        hello.sort();
        testTreeBuilder tree;
        buildPrefixTree(hello, tree);
        testAssertM(tree.output == "-G3D-base/scene/\n -demo\n -scene\n-Hello \n -G3D\n -World\n", "G3D::buildPrefixTree is broken");
