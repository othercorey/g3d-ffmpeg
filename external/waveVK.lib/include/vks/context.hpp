...
public:
    ...
    void createInstance(uint32_t version = VK_MAKE_VERSION(1, 1, 0)) {
        ...
        std::list<std::string> l_validationLayerNames = {
        ...
        if (enableValidation) {
            CStringVector r = filterLayers(l_validationLayerNames);
            layers = r;
...
protected:
    ...
    void buildDevice() {
        ...
        // Enable indexing features so we can have multiple index and vertex arrays in the shader.
