...
Array<Photon>& photonListForCurrentThread() {
    // Thread local storage implementation
    static thread_local Array<Photon> array;
    return array;
}
    ...
    if ((m_scene->lastVisibleChangeTime() > m_photonMapUpdateTime) ||
        ...
        if ((m_settings.photon.numEmitted > 0) && m_totalIndirectProducingLightPower.nonZero()) {
            ...
            
            {
                ...
                Thread::runConcurrently(0, m_settings.photon.numEmitted, [this](int){ traceOnePhoton(); }, ! m_settings.multithreaded);                
                m_photonTraceTimeMilliseconds = float((System::time() - start) / units::milliseconds());
            }
            
            {
                const RealTime start = System::time();                
                m_photonMap.insert(m_photonList);                
...
void RayTracer::traceOnePhoton() {

    Photon photon;
    emitPhoton(Random::threadCommon(), photon);
    ...
    for (int numBounces = 0; 
        ...
        if ((numBounces > 0) && surfel->nonZeroFiniteScattering()) {
            ...
            m_photonListLock.lock();
            m_photonList.append(photon);
            m_photonListLock.unlock();
        }
        ...
        surfel->scatter(PathDirection::SOURCE_TO_EYE, photon.wi, true, Random::threadCommon(), weight, wo, probabilityScale);
...
void RayTracer::traceAllPixels() {
    ...
    runConcurrently
        (Point2int32(0, 0),
         ...
         [this](Point2int32 pixel) { RayTracer::traceOnePixel(pixel); },
         !m_settings.multithreaded);
}
...
shared_ptr<Surfel> RayTracer::castRay(const Point3& X, const Vector3& w, float& distance, bool anyHit) const {
    ...
    const Ray ray(X, w);
    //TODO: does this look right? I couldnt' find a way to use maxdistance. and is anyHit meant to trigger PARTIAL_COVERAGE_THRESHOLD_ZERO? -yitong    
    return m_triTree.intersectRay(ray, (anyHit) ? TriTree::PARTIAL_COVERAGE_THRESHOLD_ZERO : 0);

    //TODO: why even have the option of treating the tritree as an array if we never do?
...
void RayTracer::traceOnePixel(Point2int32 pixel) {
    int x = pixel.x;
    int y = pixel.y;
    ...
    if (m_settings.sqrtNumPrimaryRays == 1) {
        ...
        L = traceOnePrimaryRay(float(x) + 0.5f, float(y) + 0.5f); 
    } else {
        ...
        for (int i = 0; i < m_settings.sqrtNumPrimaryRays; ++i) {
            for (int j = 0; j < m_settings.sqrtNumPrimaryRays; ++j) {
                ...
                L += traceOnePrimaryRay(float(x) + (i + 0.5f) * denom, float(y) + (j + 0.5f) * denom); 
            }
        }
...
Radiance3 RayTracer::traceOnePrimaryRay(float x, float y) {
    // Ray P, -wi
    const Ray& primaryRay = m_camera->worldRay(x, y, Rect2D::xywh(0, 0, (float)m_image->width(), (float)m_image->height()));
    return L_in(primaryRay.origin(), primaryRay.direction(), m_settings.numBackwardBounces);
}


Radiance3 RayTracer::L_in(const Point3& X, const Vector3& wi, int bouncesLeft) const {
    if (bouncesLeft == 0) {
    ...
    if (notNull(surfel)) {
        // Hit something
        return L_out(surfel, -wi, bouncesLeft);
    } else {
        // Hit background
        return BACKGROUND_RADIANCE;
    }
...
Radiance3 RayTracer::L_out(const shared_ptr<Surfel>& surfel, const Vector3& wo, int bouncesLeft) const {
    return surfel->emittedRadiance(wo) + L_scattered(surfel, wo, bouncesLeft - 1);
}


Radiance3 RayTracer::L_scattered(const shared_ptr<Surfel>& surfel, const Vector3& wo, int bouncesLeft) const {
    return
        L_direct(surfel, wo) + 
        L_indirectImpulses(surfel, wo, bouncesLeft) +
        L_indirectDiffuse(surfel, wo);
}


Radiance3 RayTracer::L_indirectImpulses(const shared_ptr<Surfel>& surfel, const Vector3& wo, int bouncesLeft) const {
    ...
    for (int i = 0; i < impulseArray.size(); ++i) {
        ...
        L += L_in(bump(surfel, impulse.direction), impulse.direction, bouncesLeft) * impulse.magnitude;
    }
...
/* TODO: I don't know if this visible() method - which depends on cpuVertexArray intersection - can be supported or is necessary. -Yitong
...
(const shared_ptr<Surfel>& surfel,
 const Vector3&            wo) const {
    ...
    */
    //TODO: I don't think we wanna use the array ever. -Yitong
    ...
    for (PhotonMap::SphereIterator photon = 
        ...
        if (s < r) {
            ...
            if (m_settings.checkFinalVisibility && f.nonZero() && (s > MIN_SURFACE_THICKNESS)) {
                ...
                const bool isVisible = visible(Y, X + offset);
                /* TODO: get rid of this stuff it turns out it's okay.
                const bool isVisible =
                    useTriTreeVisibility ? 
                        visible(Y, X + offset) :
                        ::visible(Y, X + offset, localTris, m_triTree.vertexArray());
                */
...
void RayTracer::getNearbyTris(const Point3& cullPosition, const Vector3& cullNormal, const Sphere& gatherSphere, Array<Tri>& localTri) const {
    ...
    for (int i = 0; i < localTri.size(); ++i) {
        ...
        const Vector3& n = tri.normal(m_triTree.vertexArray());
        const Point3&  V = tri.vertex(m_triTree.vertexArray(), 0).position;
...
Radiance3 RayTracer::L_direct(const shared_ptr<Surfel>& surfel, const Vector3& wo) const {
    Radiance3 L;
    const Point3& X(surfel->position);
    ...
    for (int i = 0; i < m_lighting.lightArray.size(); ++i) {
        ...
        if (light->producesDirectIllumination()) {
            ...
            if ((! light->shadowsEnabled()) || visible(Y, X, true)) {
                const Color3&      f(surfel->finiteScatteringDensity(wi, wo));
