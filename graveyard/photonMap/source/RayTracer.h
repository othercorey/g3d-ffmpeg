...
protected:
    ...
    Array<Photon>            m_photonList;
    Spinlock                 m_photonListLock;
    ...
    void traceOnePixel(Point2int32 pixel);

    /** Called from traceOnePixel */
    Radiance3 traceOnePrimaryRay(float x, float y);
    ...
    */
    Radiance3 L_in(const Point3& X, const Vector3& wi, int bouncesLeft) const;
    ...
    */
    Radiance3 L_out(const shared_ptr<Surfel>& surfel, const Vector3& wo, int bouncesLeft) const;

    /** Component of L_out due to scattered light (vs. emitted)     
      \f[  L_{\mathrm{o}, \mathrm{direct}}(X, \wo) = \sum_{Y \in \mathrm{lights}} (X, Y) \cdot f_X(\wi, \wo) \cdot |\wi \cdot \n| \f]
      for \f$\wi = \sphere(Y - X)\f$, where \f$\f$ is biradiance.
    */
    Radiance3 L_scattered(const shared_ptr<Surfel>& surfel, const Vector3& wo, int bouncesLeft) const;

    /** Component of L_scattered due to direct illumination */
    Radiance3 L_direct(const shared_ptr<Surfel>& surfel, const Vector3& wo) const;

    /** Component of L_scattered due to indirect illumination that scattered specularly */
    Radiance3 L_indirectImpulses(const shared_ptr<Surfel>& surfel, const Vector3& wo, int bouncesLeft) const;

    /** Component of L_scattered due to indirect illumination that scattered diffusely (i.e., non-specularly) */
    Radiance3 L_indirectDiffuse(const shared_ptr<Surfel>& surfel, const Vector3& wo) const;
