/**
  \file G3D-app.lib/include/G3D-app/GuiWindow.h
...
*/
#pragma once
#ifndef G3D_app_GuiWindow_h
#define G3D_app_GuiWindow_h
...
/**
   ...
   <pre>
        ...
        pane->addRadioButton("Student", Person::STUDEN, &player.role);
        pane->addRadioButton("Professor", Person::PROFESSOR, &player.role);
...
public:
    ...
    /**
      ...
      REMOVE_ON_CLOSE - Remove this GuiWindow from its containing WidgetManager and fire 
         G3D::GEvent::GUI_CLOSE with a nullptr window argument (since the window may be garbage collected before the event is received).
     */
...
protected:
    ...
    GuiWindow(const GuiText& text, const shared_ptr<GuiTheme>& skin, const Rect2D& rect, GuiTheme::WindowStyle style, CloseAction closeAction);
    ...
    /** Resolve the mouse button down event.  Called from onEvent.  
        ...
        event is in rendering coordinates (not GUI coordinates) */
    bool processMouseButtonDownEventForFocusChangeAndWindowDrag(const GEvent& event);

    /** Invoked from the default onEvent when a mouse click hits the back of a
        window that is not completely transparent. 

        \param event In rendering (not GUI) coordinates.
      */
...
public:
    ...
    /** Size of a GUI pixel in rendering pixels. Default is 1.0f
    
        Set greater than 1 on high-DPI displays to enlarge the GUI for this window only.
        Mouse events will be divided by this scale and graphics commands
        will be multiplied by it. 
        
        \sa G3D::G3DSpecification::defaultGuiWindowPixelScale
      */
    ...
    /** 
        Blocks until the dialog is closed (visible = false) \sa hideModal.  Do not call between
    ...
    /** True if this point is within the region that the window considers for event delivery. 
        ...
        \param renderingPoint in rendering pixels (not GUI pixels) */
