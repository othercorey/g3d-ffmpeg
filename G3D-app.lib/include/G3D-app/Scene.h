/**
  \file G3D-app.lib/include/G3D-app/Scene.h
...
public:
    ...
    public:
        ...
        /** Specifies whether these elements should be created as the entities are discovered.
            Overwritten by every scene loaded. */
        class Avatar {
        public:
            bool addHandEntity          = true;
            bool addControllerEntity    = true;
            bool addTorsoEntity         = true;
            Avatar() {}
            Avatar(const Any& any);
        };

        Avatar  avatar;

        VRSettings() {}
        VRSettings(const Any& any);
    };
...
protected:
    ...
    /** Simulation dependencies. 
        m_ancestorTable[Descendant] includes Ancestor.
    ...
    Table<String, LazyModelFactory>     m_modelFactory;
...
public:
    ...
    */
    ...
    /** \brief Register a new subclass of G3D::Model so that it can be constructed from a .Scene.Any file.
    ...
    */
	...
	/**
     Directories in addition to the default search directories in which loadScene() and 
     getSceneNames() will search for Scene.Any files. Called in GApp::loadScene() to add
    ...
    */
    ...
    /** Get an entity by name and downcast to the desired type.  Returns nullptr if the entity
    ...
    /** Performs very precise (usually, ray-triangle) intersection, and is much slower
        than intersectBounds.  If G3D::Model::setUseOptimizedIntersect was set to true
        before the current scene was loaded then this method will be optimized for run-time performance
        at the cost of memory and loading time. It may then be called tens of times per frame
        without significant CPU impact on frame rate.
        ...
        \param exclude any entity in this array

        \param ray The Ray in world space.

        \sa getDescendants, intersectBounds
    */
    ...
    */
    ...
    /** Returns a unique set of all (recursive) descendants of the root set, not including the root set, according to setOrder()
        \sa setOrder, clearOrder, getEntityArray */
