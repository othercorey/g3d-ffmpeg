/**
  \file G3D-app.lib/include/G3D-app/UniversalBSDF.h
...
*/
#pragma once
#ifndef G3D_app_UniversalBSDF_h
#define G3D_app_UniversalBSDF_h
...
public: 
    ...
    static inline Color3 schlickFresnel(const Color3& F0, float cos_i, float smoothness) {
        return (F0.r + F0.g + F0.b > 0.0f) ? lerp(F0, Color3(1.0f), 0.9f * pow5(square(smoothness) * (1.0f - max(0.0f, cos_i)))) : F0;
    }

    /** If the lambertian channel potentially has non-unit alpha
        or the transmission channel is non-trivially non-zero, returns true.
        Used by the ArticulatedModel OBJ loader to detect meshes
        that should be made two-sided.
    ...
    */
    ...
    /** Maps a G3D engine smoothness value to a Blinn-Phong exponent.
        ...
        The maximum exponent is clamped to 20k to avoid numerical precision
        problems in downstream computations.
     */
    static inline float smoothnessToBlinnPhongExponent(float g3dSmoothness) {
        ...
        return max(0.0f, min(20000.0f, blinnPhongExponent));
    }
    ...
    static inline float packGlossyExponent(float blinnPhongExponent) {
        ...
        // Allows perfect smoothness, which is needed for OBJ import.
        return G3D::min(g3dSmoothness, 1.0f);
