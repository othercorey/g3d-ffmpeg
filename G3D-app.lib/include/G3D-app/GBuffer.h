/**
  \file G3D-app.lib/include/G3D-app/GBuffer.h
...
public:
    ...
    public:
        ...
        enum Value {
            ...
            */
            ...
            // If you change these fields, you must also modify TriTreeBase.cpp intersectRays(...GBuffer)
    ...
    public:
        ...
        /** All fields for specific buffers default to nullptr.  In the
            future, more buffers may be added, which will also default
            to nullptr for backwards compatibility. */
        Specification();
        ...
        public:
            static size_t hashCode(const Specification& s) {
                ...
                for (int f = 0; f < Field::COUNT; ++f) {
                    h = (h << 1) | (s.encoding[f].format != nullptr);
                }
                return h;
            }
...
protected:
    ...
    [[deprecated]] String& getShaderString(const String &gbufferName, Args& args, Access access, bool &needCreation);
...
public:
    ...
    */
    ...
    CFrame& previousFrame is specific to the surface for which the arguments are being set.
    const CFrame& previousCameraFrame is previous camera frame for position change.
    const CFrame& expressivePreviousCameraFrame is the above for motion.

    RenderDevice must be passed in case of scene inversion.
    Args is the shader argument list.
    */
    virtual void setShaderArgsWritePosition
    ...
    /** Returns the Texture bound to \a f, or nullptr if there is not one */
    ...
    */
    ...
    Vector2int16 trimBandThickness() {
        return Vector2int16(m_depthGuardBandThickness - m_colorGuardBandThickness);
    }
    ...
    /**
      ...
      \param texture If nullptr, no binding is made.  Use Texture::blackIfNull and Texture::whiteIfNull to avoid this behavior.
      \param encoding Used for non-nullptr, non- Field::DEPTH_AND_STENCIL input
    */
