/**
  \file G3D-app.lib/include/G3D-app/Widget.h
...
protected:
    ...
    Widget() : m_manager(nullptr), m_depth(0.5f) {}
...
public:
    ...
    /** Called by the WidgetManager when this module is added to it.  The argument may be nullptr */
...
public:
    ...
    static shared_ptr<CallbackWidget> create() {
        return createShared<CallbackWidget>();
    }
 
    virtual void onPose
       (Array< shared_ptr<Surface> >& surfaceArray,
        Array< shared_ptr<Surface2D> >& surface2DArray) override {
        if (poseCallback) {
            poseCallback(surfaceArray, surface2DArray);
        }
    }

    virtual void onSimulation(RealTime rdt, SimTime sdt, SimTime idt) override {
        if (simulationCallback) {
            simulationCallback(rdt, sdt, idt);
        }
    }

    virtual void onBeforeGraphics() override {
        if (beforeGraphicsCallback) { beforeGraphicsCallback(); }
    }
...
public:
    ...
    /** At most one Widget has focus at a time.  May be nullptr. 
    ...
    /** The Widget must have already been added.  This widget will be moved to
        ...
        You can set the focussed widget to nullptr.
