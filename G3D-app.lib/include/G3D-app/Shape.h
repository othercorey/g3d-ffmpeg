/**
  \file G3D-app.lib/include/G3D-app/Shape.h
...
class MeshShape : public Shape {
    ...
    /** If not null, the bspTree, _boundingSphere, and _boundingAABox have been initialized. */
    mutable shared_ptr<TriTree> _bspTree;
    ...
    void init(const CPUVertexArray& vertexArray, const Array<Tri>& tri, float scale = 1.0f);
...
public:
    ...
    explicit MeshShape(const String& filename, float scale = 1.0f);

    /** Copies the triangle array */
    MeshShape(const CPUVertexArray& vertexArray, const Array<Tri>& tri);

    explicit MeshShape(const class ParseOBJ& parseOBJ);
