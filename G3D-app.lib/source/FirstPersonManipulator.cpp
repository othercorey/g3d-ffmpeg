/**
  \file G3D-app.lib/source/FirstPersonManipulator.cpp
...
void FirstPersonManipulator::setEnabled(bool a) {
    ...
    debugAssertM(m_userInput != nullptr, 
...
void FirstPersonManipulator::onSimulation(RealTime rdt, SimTime sdt, SimTime idt) {
    ...
    if (window()->numJoysticks() > 0) {
        wantUp   = wantUp   || m_userInput->keyDown(GKey::CONTROLLER_DPAD_UP);
        wantDown = wantDown || m_userInput->keyDown(GKey::CONTROLLER_DPAD_DOWN);
        wantSlow = wantSlow || m_userInput->keyDown(GKey::CONTROLLER_LEFT_BUMPER);
        wantFast = wantFast || m_userInput->keyDown(GKey::CONTROLLER_RIGHT_BUMPER);

        joyRotate = m_userInput->rightStick();
...
bool FirstPersonManipulator::onEvent(const GEvent& event) {
    if (m_enabled && 
        ...
        if ((m_userInput != nullptr) && (event.button.button == 0)) {
            // "Right click"
