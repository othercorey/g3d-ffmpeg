/**
  \file G3D-app.lib/source/HeightfieldModel.cpp
...
void HeightfieldModel::setShaderArgs(Args& args) const {
    // Heightfield args
    args.setAttributeArray("position", m_positionArray);
    args.setIndexStream(m_indexStream);
    args.setUniform("elevation", m_elevation, Sampler::video());
    ...
    args.setUniform("pixelsPerQuadSide", float(m_specification.pixelsPerQuadSide));
    args.setUniform("scale", Vector3(metersPerQuad, m_specification.maxElevation, metersPerQuad));

    // Material texture coordinates per meter
    args.setUniform("texCoordsPerMeter", 1.0f / m_specification.metersPerTexCoord);
...
void HeightfieldModel::pose
    ...
    for (Point2int32 t(0,0); t.y < numTiles.y; ++t.y) {
        for (t.x = 0; t.x < numTiles.x; ++t.x) {
            surfaceArray.append(std::make_shared<Tile>(this, t, rootFrame, prevFrame, entity, e));
        }
    }
}
        ...
        */
        ...
        if (notCulled) {
            ...
            if (firstIntersectDistance < maxDistance) {
                ...
                info.set
                    (dynamic_pointer_cast<HeightfieldModel>(const_cast<HeightfieldModel*>(this)->shared_from_this()), 
