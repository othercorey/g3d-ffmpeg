/**
  \file G3D-app.lib/source/GFont.cpp
...
void GFont::renderCharVertexArray(RenderDevice* renderDevice, const Array<CPUCharVertex>& cpuCharArray, Array<int>& indexArray) const {
    ...
    renderDevice->setBlendFunc(RenderDevice::BLEND_ONE, RenderDevice::BLEND_ONE_MINUS_SRC_ALPHA);
...
Vector2 GFont::draw2D
   (RenderDevice*               renderDevice,
    ...
    debugAssert(renderDevice != nullptr);
...
Vector2 GFont::draw3DBillboard
   (RenderDevice*               renderDevice,
    ...
    return draw3D(renderDevice, s, CFrame(renderDevice->cameraToWorldMatrix().rotation, pos3D), size, color, border, xalign, yalign, spacing);
}
