/**
  \file G3D-app.lib/source/MD3Model.cpp
...
void MD3Model::Skin::loadSkinFile(const String& filename, PartSkin& partSkin) {
    ...
    for (int lineIndex = 0; lineIndex < lines.length(); ++lineIndex) {
        ...
        if (textureName == "nodraw") {
            // Intentionally nullptr material
...
bool MD3Part::intersect
   (const Ray& r, 
    ...
    for (int i = 0; i < m_triListArray.length(); ++i) {
        ...
        const float intersectBounds = intersectingRay.intersectionTime(bounds1);
        if (intersectBounds < maxDistance) {
            ...
            for (int k = 0; k < triList.m_indexArray.length(); k += 3) {
                ...
                const float testTime = intersectingRay.intersectionTime(p[0], p[1], p[2], w0, w1, w2);
                if (testTime < maxDistance) {
                    maxDistance = testTime;
                    hit = true;
                    info.set
                    (
                        dynamic_pointer_cast<MD3Model>(const_cast<MD3Model*>(model)->shared_from_this()),
...
void MD3Part::loadSurface(BinaryInput& bi, TriList& triList) {
    ...
    // Currently discarding shader name and index
        ...
        if (! shaderName.empty() && FileSystem::exists(FilePath::concat(m_modelDir, shaderName))) {
...
void MD3Model::loadSpecification(const Specification& spec) {
    ...
    loadAnimationCfg(FilePath::concat(spec.directory, "animation.cfg"));
...
(Array<shared_ptr<Surface> >&   surfaceArray,
 ...
 const Surface::ExpressiveLightScatteringProperties& e) {
    const MD3Model::Pose& p    = pose     ? *dynamic_cast<const MD3Model::Pose*>(pose)     : MD3Model::Pose();
    MD3Model::pose(surfaceArray, rootFrame, prevFrame, p, entity, e);
}


void MD3Model::pose
(Array<shared_ptr<Surface> >& posedModelArray, 
 ...
 const Surface::ExpressiveLightScatteringProperties& e) {
    ...
    posePart(PART_LOWER, pose, posedModelArray, baseFrame, previousBaseFrame, entity, e);
    ...
    posePart(PART_UPPER, pose, posedModelArray, baseFrame, previousBaseFrame, entity, e);
    ...
    posePart(PART_HEAD, pose, posedModelArray, baseFrame, previousBaseFrame, entity, e);
}
...
bool MD3Model::intersect
    ...
    static const MD3Model::Pose defaultPose;
    ...
    for (int partType = 0; partType < NUM_PARTS; ++partType) {
        ...
        const bool justhit = m_parts[partType]->intersect(ray, bframe, frame1, frame2, alpha, maxDistance, partType, this, info, entity, skin);
...
void MD3Model::posePart(PartType partType, const Pose& pose, Array<shared_ptr<Surface> >& posedModelArray, const CFrame& cframe, const CFrame& previousFrame, const shared_ptr<Entity>& entity,
    ...
    for (int surfaceIndex = 0; surfaceIndex < part->m_triListArray.length(); ++surfaceIndex) {
        ...
        if (isNull(material)) {
            // triLists with an intentionally nullptr material do not render
            continue;
        }
