/**
  \file G3D-app.lib/source/GuiScrollBar.cpp
...
static const float BUTTON_HEIGHT_AND_WIDTH = 15.0f;
static const float BUTTON_SCROLL_SPEED = .03f;
static const float BUTTON_PRESS_SCROLL = .01f;
...
bool GuiScrollBar::onEvent(const GEvent& event) {
    ...
    if (event.type == GEventType::MOUSE_BUTTON_DOWN) {
        ...
        const Vector2& mouse = Vector2(event.motion.x, event.motion.y);
