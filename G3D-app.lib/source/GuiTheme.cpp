/**
  \file G3D-app.lib/source/GuiTheme.cpp
...
void GuiTheme::beginRendering(RenderDevice* rd, float pixelScale) {
    ...
    CFrame S;
    S.rotation[0][0] = S.rotation[1][1] = m_pixelScale;
    rd->setObjectToWorldMatrix(S);
...
void GuiTheme::renderTextBox
    ...
    float fontSize = m_textBox.contentStyle.size;
    ...
    if (text.numElements() == 1) {
        ...
        fontSize = element.size(m_textBox.contentStyle.size);
    ...
    const String& textBeforeCursor = fullText.substr(0, cursorPosition);
    const Vector2& boundsBeforeCursor = font->bounds(textBeforeCursor, fontSize);

    // Slide the text backwards so that the cursor is visible
    const float textRenderOffset = -max(0.0f, boundsBeforeCursor.x - clientArea.width());
    ...
    if (highlightPosition != -1) {
        // Swap marker/cursor if needed to render left to right
        const Vector2& boundsBeforeMarker = font->bounds(fullText.substr(0, highlightPosition));
        const float highlightStartX = min(boundsBeforeMarker.x, boundsBeforeCursor.x);
        const float highlightEndX = max(boundsBeforeMarker.x, boundsBeforeCursor.x);

        // Render highlight between highlight marker and cursor
        Rect2D highlightBounds = Rect2D::xywh(textRenderOffset + highlightStartX, 0, highlightEndX - highlightStartX, clientArea.height());
        renderSelection(highlightBounds);
    }

    // Draw main text
    addDelayedText(font, fullText, Vector2(textRenderOffset, clientArea.height() / 2), fontSize, color,
    ...
    if (focused) {
        addDelayedText(cursor,
                       ...
                       Vector2(textRenderOffset + boundsBeforeCursor.x, clientArea.height() / 2),
...
void GuiTheme::renderMultiLineTextBox
    (const Rect2D&      fullBounds, 
     bool               enabled, 
     bool               focused, 
     const GuiText&     caption,
    ...
    // Draw each line within client area
    const Array<String>& lines = splitLines(fullText);
    if (lines.length() > 0) {
        // Calculate maximum lines that fit in client area
        const float lineHeight = font->bounds(" ", fontSize).y;
        const int linesPerArea = iCeil(clientArea.height() / lineHeight);
    
        // Find line containing cursor position
        int cursorLine = 0;
        for (int c = 0; cursorLine < lines.length(); ++cursorLine) {
            // For each line, total all previous lines and check if cursor is in range
            const int countUpToThisLine = c;
            c += (int)lines[cursorLine].length() + 1;
            if (cursorPosition < c) {
                // When in range, subtract total up to this line
                // to make cursorPosition an offset into the line containing the cursor
                cursorPosition -= countUpToThisLine;
                break;
            }
        }

        const String& textBeforeCursor = lines[cursorLine].substr(0, cursorPosition);
        const Vector2& boundsBeforeCursor = font->bounds(textBeforeCursor, fontSize);
        ...
        const int firstRenderedLine = max(cursorLine - linesPerArea + 1, 0);
        const int lastRenderedLine = min(firstRenderedLine + linesPerArea, lines.length());

        if (highlightPosition != -1) {
            // Find line containing highlight marker
            int markerLine = 0;
            for (int c = 0; markerLine < lines.length(); ++markerLine) {
                // For each line, total all previous lines and check if marker is in range
                const int countUpToThisLine = c;
                c += (int)lines[markerLine].length() + 1;
                if (highlightPosition < c) {
                    // When in range, subtract total up to this line
                    // to make highlightPosition an offset into the line containing the marker
                    highlightPosition -= countUpToThisLine;
                    break;
                }
            }

            // Clamp as mouse click can happen outside text
            markerLine = min(markerLine, lines.length() - 1);
            const Vector2& boundsBeforeMarker = font->bounds(lines[markerLine].substr(0, highlightPosition), fontSize);
            ...
            if (markerLine == cursorLine) {
                ...
                // Render highlight between highlight marker and cursor
                Rect2D highlightBounds = Rect2D::xywh(textRenderOffset + highlightStartX, lineHeight * (markerLine - firstRenderedLine), highlightEndX - highlightStartX, lineHeight);
                renderSelection(highlightBounds);
            } else {
                // Swap marker/cursor if needed to render left-to-right top-to-bottom
                const int highlightStartLine = min(markerLine, cursorLine);
                const int highlightEndLine = max(markerLine, cursorLine);

                // Render first line from from marker/cursor to end of line
                const float firstLineStartX = markerLine > cursorLine ? boundsBeforeCursor.x : boundsBeforeMarker.x;
                const float firstLineEndX = font->bounds(lines[highlightStartLine], fontSize).x;
                Rect2D highlightBounds = Rect2D::xywh(textRenderOffset + firstLineStartX, lineHeight * (highlightStartLine - firstRenderedLine), firstLineEndX - firstLineStartX, lineHeight);
                renderSelection(highlightBounds);

                // Render any lines between highlight and cursor line completely
                for (int l = highlightStartLine + 1; l < highlightEndLine; ++l) {
                    Vector2 fullLine = font->bounds(lines[l], fontSize);
                    highlightBounds = Rect2D::xywh(textRenderOffset, lineHeight * (l - firstRenderedLine), fullLine.x, lineHeight);
                    renderSelection(highlightBounds);
                }

                // Render last line up to marker/cursor position
                const float lastLineEndX = markerLine > cursorLine ? boundsBeforeMarker.x : boundsBeforeCursor.x;
                highlightBounds = Rect2D::xywh(textRenderOffset, lineHeight * (highlightEndLine - firstRenderedLine), lastLineEndX, lineHeight);
                renderSelection(highlightBounds);
            }
        }

        // Draw main text
        for (int l = firstRenderedLine; l < lastRenderedLine; ++l) {
            addDelayedText(font, lines[l], Vector2(textRenderOffset, lineHeight * (l - firstRenderedLine)), fontSize, color,
                            outlineColor, GFont::XALIGN_LEFT, GFont::YALIGN_TOP);
        }
        ...
        if (focused) {
            addDelayedText(cursor,
                           m_textBox.contentStyle,
                           Vector2(textRenderOffset + boundsBeforeCursor.x, lineHeight * (cursorLine - firstRenderedLine)),
                           GFont::XALIGN_CENTER, GFont::YALIGN_TOP);
        }
    } else {
        // Draw cursor at 0,0
        if (focused) {
...
Rect2D GuiTheme::checkBoxToClickBounds(const Rect2D& bounds, const GuiText& caption) const {
    Vector2 captionBounds = this->bounds(caption);
    Rect2D clickBounds = Rect2D::xywh(bounds.x0y0(), Vector2(m_checkBox.width() + captionBounds.x, m_checkBox.height()));
    return clickBounds;
}

Rect2D GuiTheme::radioButtonToClickBounds(const Rect2D& bounds, const GuiText& caption) const {
    Vector2 captionBounds = this->bounds(caption);
    Rect2D clickBounds = Rect2D::xywh(bounds.x0y0(), Vector2(m_radioButton.width() + captionBounds.x, m_radioButton.height()));
    return clickBounds;
}
...
void GuiTheme::pushClientRect(const Rect2D& newRelativeClientRect) {
    debugAssert(! newRelativeClientRect.isEmpty());
    debugAssert(m_inRendering);
    ...
    const Rect2D& newAbsoluteRect = newRelativeClientRect * m_pixelScale + oldMatrix.translation.xy();
    const Rect2D& afterIntersectRect = oldRect.intersect(newAbsoluteRect);
    m_rd->setClip2D(afterIntersectRect);

    const CoordinateFrame& newMatrix = oldMatrix * CoordinateFrame(Vector3(newRelativeClientRect.x0y0(), 0));
...
void GuiTheme::Window::render
   (RenderDevice*                   rd,
    const Rect2D&                   bounds,
    bool                            isFocused,
    Array<GuiTheme::Rectangle>&     delayedRectangles) const {

    const Vector2& offset = isFocused ? focused : defocused;
...
void GuiTheme::StretchRectHV::render
   (RenderDevice*                   rd,
    const Rect2D&                   bounds,
    const Vector2&                  texOffset,
    Array<GuiTheme::Rectangle>&     delayedRectangles) const {
    
    const float topHeight    = top.left.height();
    const float bottomHeight = bottom.left.height();
    const float centerHeight = max(0.0f, bounds.height() - topHeight - bottomHeight);

    top.render(rd, Rect2D::xywh(bounds.x0y0(), Vector2(bounds.width(), topHeight)),
               texOffset, delayedRectangles);

    centerLeft.render(rd, Rect2D::xywh(bounds.x0y0() + Vector2(0, topHeight), Vector2(top.left.width(), centerHeight)),
                      texOffset, delayedRectangles);

    centerCenter.render(rd, Rect2D::xywh(bounds.x0y0() + Vector2(top.left.width(), topHeight), Vector2(max(0.0f, bounds.width() - (top.left.width() + top.right.width())), centerHeight)),
                        texOffset, delayedRectangles);

    centerRight.render(rd, Rect2D::xywh(bounds.x1y0() + Vector2(-top.right.width(), topHeight), Vector2(top.right.width(), centerHeight)),
                      texOffset, delayedRectangles);

    bottom.render(rd, Rect2D::xywh(bounds.x0y0() + Vector2(0, topHeight + centerHeight), Vector2(bounds.width(), bottomHeight)),
                  texOffset, delayedRectangles);
}
