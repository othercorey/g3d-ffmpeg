/**
  \file G3D-app.lib/source/BSPMAPLoad.cpp
...
/**
  ...
  <P>
    ...
    grid, regions of 3x3 vertices represent biquadratic Bezier patches. Adjacent 
...
#       endif
    ...
    Map* m = new Map();
    if (m->load(FilePath::concat(path, ""), fileName, altLoad, defaultTextureFile)) {
...
//#pragma message("Fix destructive useage of strtok in BSPMAPLoad.cpp ")
            ...
            if(pt){
                ...
                pos.z =    -atoi(strtok(nullptr," "))       * G3D_LOAD_SCALE;
                pos.y =     atoi(strtok(nullptr,"\""))   * G3D_LOAD_SCALE;
    ...
    try {
        for (int p = 0; p < numPath; ++p) {
            for (int i = 0; i < numExt; ++i) {
                const String& full = FilePath::concat(path[p], filename) + ext[i];
