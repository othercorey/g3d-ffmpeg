/**
  \file G3D-app.lib/source/MD2Model_load.cpp
...
void MD2Model::Part::load(const String& filename, float resize) {
    ...
    setName(FilePath::baseExt(filename));
    ...
    const shared_ptr<VertexBuffer>& indexBuffer = VertexBuffer::create(indexArray.size() * sizeof(int), VertexBuffer::WRITE_ONCE);
...
void MD2Model::Part::computeTexCoords(const Array<Vector2int16>& inCoords) {
