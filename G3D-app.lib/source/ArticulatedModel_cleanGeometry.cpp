/**
  \file G3D-app.lib/source/ArticulatedModel_cleanGeometry.cpp
...
void ArticulatedModel::computeBounds() {
    ...
    // Rebuild tri trees in parallel across all meshes
    runConcurrently(0, m_meshArray.size(), [&](int m) {
        Mesh* mesh = m_meshArray[m];
        mesh->triTree = nullptr;
        if (Model::useOptimizedIntersect()) {
            const int numIndices = mesh->cpuIndexArray.size();
            const int numTris = numIndices / 3;
            const int* index = mesh->cpuIndexArray.getCArray();
            const CPUVertexArray& vertexArray = mesh->geometry->cpuVertexArray;

            // Don't build tri trees for very small meshes or ones with joints
            if ((mesh->contributingJoints.size() == 1) && (numTris >= 40) && isNull(mesh->triTree)) {
                mesh->triTree = TriTree::create(false);

                // Flatten the vertex array to just the portion needed for this mesh.
                // This will create a 3-6x increase in the number of vertices (because there's no reuse)
                // but avoids duplicating the entire vertex array of the Geometry for each Mesh that uses it.
                Array<Tri> triArray;
                triArray.resize(numTris);
                CPUVertexArray flattenedVertexArray;
                flattenedVertexArray.vertex.resize(numIndices);
                const bool twoSided = mesh->twoSided;
                for (int t = 0; t < numTris; ++t) {
                    const int i = 3 * t;
                    for (int j = 0; j < 3; ++j) {
                        flattenedVertexArray.vertex[i + j] = vertexArray.vertex[index[i + j]];
                    }
                    triArray[t] = Tri(i, i + 1, i + 2, flattenedVertexArray, nullptr, twoSided, false);
                }
                mesh->triTree->setContents(triArray, flattenedVertexArray, ImageStorage::IMAGE_STORAGE_CURRENT);
            }
        }
    });
}
...
void ArticulatedModel::Geometry::cleanGeometry(const CleanGeometrySettings& settings, const Array<Mesh*>& meshes) {
    ...
    timer.printElapsedTime("  determineCleaningNeeds");
    ...
    if (computeSomeNormals || (settings.forceVertexMerging && settings.allowVertexMerging)) {
        ...
        if (settings.allowVertexMerging) {
            ...
            timer.printElapsedTime("  mergeVertices");
    ...
    timer.printElapsedTime("  deallocation of adjacentFaceTable");

    if (computeSomeTangents) {
        // Compute tangent space
        computeMissingTangents(affectedMeshes);
        timer.printElapsedTime("  computeMissingTangents");
    }
