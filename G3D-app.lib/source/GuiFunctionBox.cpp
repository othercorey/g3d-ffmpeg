/**
  \file G3D-app.lib/source/GuiFunctionBox.cpp
...
void GuiFunctionBox::render(RenderDevice* rd, const shared_ptr<GuiTheme>& theme, bool ancestorsEnabled) const {
    ...
    theme->pauseRendering();
    {
        ...
        rd->intersectClip2D(Rect2D::xywh(m_clipBounds.x0y0() * scale + matrix.translation.xy(), m_clipBounds.wh() * scale));
...
void GuiFunctionBox::drawSpline(RenderDevice* rd, const shared_ptr<GuiTheme>& theme) const {
    (void)theme;
    int N = max((int)(m_bounds.width() / 4), 30);
...
void GuiFunctionBox::drawControlPoints(RenderDevice* rd, const shared_ptr<GuiTheme>& skin) const {
    (void)skin;
    const float size = 10.0f * GLFWWindow::defaultGuiPixelScale();   
    const int coverageSamples = 8;
    ...
    mesh.render(rd, coverageSamples);
}
