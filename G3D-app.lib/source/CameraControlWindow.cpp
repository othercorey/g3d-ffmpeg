/**
  \file G3D-app.lib/source/CameraControlWindow.cpp
...
const Vector2 CameraControlWindow::sExpandedWindowSize(286 + 16, 46 + 440);
    ...
    m_expanded(false)
{
    ...
    GuiPane* _filmPane = tabPane->addTab(GuiText("Film", defaultFont, (float)tabCaptionSize));
    GuiScrollPane* filmScroll = _filmPane->addScrollPane(true, false, GuiTheme::BORDERLESS_SCROLL_PANE_STYLE);
    filmScroll->moveBy(-6, 0);
    GuiPane* filmPane = filmScroll->viewPane();
    {
        ...
        filmScroll->setSize(286 + 10 + 6, 400);
        _filmPane->pack();
    }
...
#   define INDENT_SLIDER(n) n->setWidth(275); n->moveBy(15, 0); n->setCaptionWidth(100);
    ...
    {        
        ...
        GuiNumberBox<int>* b = focusPane->addNumberBox("Res. Reduction", Pointer<int>(&m_camera->depthOfFieldSettings(),
...
#   undef INDENT_SLIDER
    ...
    projectionPane->moveBy(-3, 2);
    {
        ...
        // Field of view
        b = projectionPane->addNumberBox("Field of View", Pointer<float>(m_camera, &Camera::fieldOfViewAngleDegrees, &Camera::setFieldOfViewAngleDegrees), GuiText("\xB0", greekFont, 15), GuiTheme::LOG_SLIDER, 10.0f, 360.0f, 0.5f);
        b->setWidth(290);  b->setCaptionWidth(105);
...
(const shared_ptr<FirstPersonManipulator>&      manualManipulator,
 ...
 const shared_ptr<GuiTheme>&                    theme) {
    ...
    return shared_ptr<CameraControlWindow>(new CameraControlWindow(manualManipulator, trackManipulator, cameraManipulator, camera, theme));
}
...
void CameraControlWindow::updateTrackFiles() {
    ...
    m_trackFileIndex = min(m_trackFileArray.size() - 1, m_trackFileIndex);
}
...
void CameraControlWindow::saveSpline(const String& trackName) {
    ...
    m_trackFileIndex = max(0, m_trackFileArray.findIndex(trackName));
