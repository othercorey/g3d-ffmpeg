/**
  \file G3D-app.lib/source/Shape.cpp
...
MeshShape::MeshShape(const String& filename, float scale) {
    const shared_ptr<ArticulatedModel>& model = ArticulatedModel::fromFile(filename);
    ...
    init(vertexArray, triArray, scale);
}


MeshShape::MeshShape(const class ParseOBJ& parseObj) {
...
MeshShape::MeshShape(const CPUVertexArray& vertexArray, const Array<Tri>& tri) {
    init(vertexArray, tri);
}


void MeshShape::init(const CPUVertexArray& vertexArray, const Array<Tri>& tri, float scale) {
    ...
    for (int t = 0; t < tri.size(); ++t) {
        for (int v = 0; v < 3; ++v, ++i) {
            _indexArray[i] = i;
            _vertexArray[i] = tri[t].position(vertexArray, v) * scale;
        }
    }
...
Vector3 MeshShape::randomInteriorPoint(Random& rnd) const {
    ...
    for (int attempt = 0; attempt < MAX_ATTEMPTS_RANDOM_INTERIOR_POINT; ++attempt) {
        ...
        while (_bspTree->intersectRay(r, hit, TriTree::DO_NOT_CULL_BACKFACES)) {
