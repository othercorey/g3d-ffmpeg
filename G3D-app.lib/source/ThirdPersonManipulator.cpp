/**
  \file G3D-app.lib/source/ThirdPersonManipulator.cpp
...
void UIGeom::computeProjection(RenderDevice* rd) {
    ...
    for (int i = 0; i < line3D.size(); ++i) {
        ...
        for (int j = 0; j < line.numVertices(); ++j) {
            ...
            // Note that this vector may be NaN or inf
            if (! v.xy().isFinite()) {
                v = Vector4::zero();
            }
            debugAssert(v.xy().isFinite());
            debugAssert(isFinite(v.z));
            debugAssert(isFinite(v.w));
            vertex[j] = v.xy();
            depth[j] = v.z;
            w[j] = v.w;
        }
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                for (int v = 0; v < line.numVertices(); ++v) {
                    ...
                    if (N.squaredLength() < 0.05) {
                        ...
                        mesh.setNormal(normal[min(v, normal.size() - 1)]);
                    } else {
...
public:
    ...
    virtual void getCoordinateFrame(CoordinateFrame& c, bool previous = false) const override {
        ...
        debugAssert(! c.translation.isNaN());
    }
...
Vector3 ThirdPersonManipulator::singleAxisTranslationDrag(int a, const Vector2& delta) {
    ...
    const Vector2& vec = axis.point(1) - axis.point(0);
    const float length2 = max(0.5f, vec.squaredLength());

    // Divide by squared length since we not only normalize but need to take into
    // account the angular foreshortening.
    const float distance = vec.dot(delta) / length2;

    debugAssert(isFinite(distance));
    debugAssert(m_controlFrame.rotation.column(a).isFinite());
...
void ThirdPersonManipulator::onDrag(const Vector2& delta) {
    debugAssert(! m_controlFrame.translation.isNaN());
    debugAssert(! m_offsetFrame.translation.isNaN());
    if ((m_dragAxis >= XY) && (m_dragAxis <= ZX)) {
        ...
        // on screen.
        const float angle = delta.dot(m_dragTangent) * 0.00004f / m_dragW;

        // Axis about which to rotate
        Vector3 axis;
        axis[m_dragAxis - RX] = 1.0f;

        const Matrix3& R = Matrix3::fromAxisAngle(axis, angle);

        m_controlFrame.rotation *= R;
...
ThirdPersonManipulator::ThirdPersonManipulator() : 
    ...
    m_posedModel = TPMSurface::createShared<TPMSurface>(this);
    debugAssert(! m_controlFrame.translation.isNaN());
    debugAssert(! m_offsetFrame.translation.isNaN());
}
