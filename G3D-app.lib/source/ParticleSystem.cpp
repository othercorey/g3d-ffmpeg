/**
  \file G3D-app.lib/source/ParticleSystem.cpp
...
static shared_ptr<Texture> convertToTextureArray(const String& name, const shared_ptr<Texture>& tex) {
    const shared_ptr<Texture>& result = Texture::createEmpty(name, tex->width(), tex->height(), tex->encoding(), Texture::DIM_2D_ARRAY, false, 1);
...
static shared_ptr<Texture> layeredTextureConcatenate(
    ...
    for (int i = 0; i < numLayers - 1; ++i) {
        ...
        Texture::copy(previousLayers, result, 0, 0, 1.0f, Vector2int16(0, 0), 
            CubeFace::POS_X, CubeFace::POS_X, nullptr, false, prevLayerIndex, i);
    }

    Texture::copy(newLayer, result, 0, 0, 1.0f, Vector2int16(0, 0),
        CubeFace::POS_X, CubeFace::POS_X, nullptr, false, 0, numLayers - 1);
    result->generateMipMaps();
...
int ParticleMaterial::insertMaterial(const shared_ptr<UniversalMaterial>& newMaterial) {
    ...
    const shared_ptr<Texture>& lambertian      = newMaterial->bsdf()->lambertian().texture();
    const shared_ptr<Texture>& glossy          = newMaterial->bsdf()->glossy().texture();
    const shared_ptr<Texture>& transmissive    = newMaterial->bsdf()->transmissive().texture();
    const shared_ptr<Texture>& emissive        = newMaterial->emissive().texture();
    int layerIndex = 0;
    ...
    if (isNull(s_material)) { 
        ...
        const shared_ptr<Texture> currentLambertian   = s_material->bsdf()->lambertian().texture();
        const shared_ptr<Texture> currentGlossy       = s_material->bsdf()->glossy().texture();
        const shared_ptr<Texture> currentTransmissive = s_material->bsdf()->transmissive().texture();
        const shared_ptr<Texture> currentEmissive     = s_material->emissive().texture();
...
shared_ptr<ParticleMaterial> ParticleMaterial::create(const shared_ptr<UniversalMaterial>& material) {
    ...
    const int index = insertMaterial(material);
    return createShared<ParticleMaterial>(index, material->bsdf()->lambertian().texture()->width());
}
...
void ParticleSystem::updateBounds() {   
    ...
    debugAssert(! m_lastObjectSpaceAABoxBounds.low().isNaN() && ! m_lastObjectSpaceAABoxBounds.high().isNaN());
...
void ParticleSystem::applyPhysics(float t, float dt) {
    ...
    for (int i = 0; i < m_particle.size(); ++i) {
        ...
        debugAssert(relativeVelocity.squaredMagnitude() < 1e12);
        ...
        const Vector3& acceleration = gravitationalAcceleration + dragForce / max(P.mass, 0.1f);
        
        P.velocity += acceleration * dt;
        P.position += P.velocity * dt;
        P.angle += P.angularVelocity * dt;

        debugAssert(P.position.isFinite());
        debugAssert(P.velocity.isFinite());
    }
