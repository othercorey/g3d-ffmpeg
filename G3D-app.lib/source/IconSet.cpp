/**
  \file G3D-app.lib/source/IconSet.cpp
...
void IconSet::findImages(const String& baseDir, const String& sourceDir, Array<Source>& sourceArray) {
    ...
    FileSystem::getFiles(FilePath::concat(pathConcat(baseDir, sourceDir), "*"), filenameArray);
    for (int i = 0; i < filenameArray.size(); ++i) {
        if (Image::fileSupported(filenameArray[i])) {
            String f = FilePath::concat(sourceDir, filenameArray[i]);
            shared_ptr<Image> im = Image::fromFile(FilePath::concat(baseDir, f));
    ...
    FileSystem::getDirectories(FilePath::concat(FilePath::concat(baseDir, sourceDir), "*"), dirArray);
    for (int i = 0; i < dirArray.size(); ++i) {
        if (dirArray[i] != ".svn" && dirArray[i] != "CVS" && dirArray[i] != ".git") {
            findImages(baseDir, FilePath::concat(sourceDir, dirArray[i]), sourceArray);
        }
    }
}
...
void IconSet::makeIconSet(const String& baseDir, const String& outFile) {
    ...
    for (int i = 0; i < sourceArray.size(); ++i) {
        ...
        if (s.channels == maxChannels) {
            src.load(FilePath::concat(baseDir, s.filename));
        } else {
            // Need to expand the number of channels
            GImage tmp(FilePath::concat(baseDir, s.filename));
