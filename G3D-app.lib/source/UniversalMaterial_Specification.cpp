/**
  \file G3D-app.lib/source/UniversalMaterial_Specification.cpp
...
UniversalMaterial::Specification::Specification(const Any& any) {
    *this = Specification();
    ...
    if ((any.type() == Any::STRING) && (endsWith(toLower(any.string()), ".universalmaterial.any"))) {
        // Parse the file
        *this = UniversalMaterial::Specification(Any::fromFile(any.resolveStringAsFilename()));
        return;
    }
    ...
    for (Any::AnyTable::Iterator it = any.table().begin(); it.isValid(); ++it) {
        ...
        if (key == "lambertian") {
            ...
            any.verify(false, "Illegal key in UniversalMaterial::Specification: " + it->key);
        }
    }
}
...
(const String&              filename, 
 ...
 bool                       invertPrecomputedNormalYAxis) {
     ...
     m_bump.texture.preprocess.bumpMapPreprocess.zExtentPixels = normalMapWhiteHeightInPixels;
     if (invertPrecomputedNormalYAxis) {
         m_bump.texture.preprocess.modulate = Color4(1, -1, 1, 1);
         m_bump.texture.preprocess.offset = Color4(0, 1, 0, 0);
     }
