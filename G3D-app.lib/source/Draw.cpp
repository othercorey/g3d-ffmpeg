/**
  \file G3D-app.lib/source/Draw.cpp
...
static void drawSpotLight(shared_ptr<Light> light, RenderDevice* rd){
    ...
    rd->pushState(); {
        ...
        const float distance = clamp(sqrt(LIGHT_RADIUS) * 15.0f, 0.1f, 0.75f);//clamp(light->effectSphere().radius * 0.08f, 0.1f, 1.5f);
        const float nearAlpha = 0.35f;
        const float innerRadius = LIGHT_RADIUS * 0.5f;
        ...
        SlowMesh slowMesh(PrimitiveType::TRIANGLES); {
            if (square) {
                ...
                for(int i = 0; i < 4; ++i) {
                    ...
                    slowMesh.setColor(Color4(color, nearAlpha));
...
(const Rect2D&       rect,
 ...
 float               gammaAdjust) {
    ...
    args.setUniform("gammaAdjust", gammaAdjust);
