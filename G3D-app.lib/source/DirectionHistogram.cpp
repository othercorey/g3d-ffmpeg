/**
  \file G3D-app.lib/source/DirectionHistogram.cpp
...
void DirectionHistogram::insert(const Vector3& vector, float weight) {
    // Find the quad hit
    TriTree::Hit hit;
    if (m_tree->intersectRay(Ray(Point3::zero(), vector.direction()), hit, 0)) {
        ++m_numSamples;

        // Hit
        const int index = (*m_tree)[hit.triIndex].data<VertexIndexIndex>()->index;
