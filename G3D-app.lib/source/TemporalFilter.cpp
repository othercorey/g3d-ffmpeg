/**
  \file G3D-app.lib/source/TemporalFilter.cpp
...
shared_ptr<Texture> TemporalFilter::apply
   (RenderDevice*                   rd, 
    ...
    if (isNull(m_previousDepthBuffer) || 
        ...
        m_resultFramebuffer = Framebuffer::create(Texture::createEmpty("G3D::TemporalFilter::m_resultFramebuffer", m_previousTexture->width(), m_previousTexture->height(), m_previousTexture->format()));
