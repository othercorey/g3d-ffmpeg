/**
  \file G3D-app.lib/source/ShadowMap.cpp
 ...
 const RenderPassType               passType) {
    ...
    for (int i = 0; i < shadowCaster.length(); ++i) {
        ...
        case RenderPassType::OPAQUE_SHADOW_MAP:
            ...
            needsToRenderThisPass = (c->transparencyType() != TransparencyType::ALL);
            break;

        case RenderPassType::TRANSPARENT_SHADOW_MAP:
            needsToRenderThisPass = (c->transparencyType() == TransparencyType::SOME) || (c->transparencyType() == TransparencyType::ALL);
            break;
