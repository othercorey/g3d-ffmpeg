/**
  \file G3D-app.lib/source/SceneEditorWindow.cpp
...
public:
    ...
    virtual float positionalEventZ(const Point2& pixel) const override {
        ...
        const Ray& ray = m_sceneEditor->m_scene->eyeRay
            (m_sceneEditor->m_app->activeCamera(), 
    ...
    bool onEvent(const GEvent& event) override {
        ...
        if ((event.type == GEventType::MOUSE_BUTTON_DOWN) && (event.button.button == 0) && ! event.button.controlKeyIsDown) {
            ...
            const Ray& ray = scene->eyeRay(m_sceneEditor->m_app->activeCamera(), event.mousePosition() + Vector2(0.5f, 0.5f),
                                           view, m_sceneEditor->m_app->settings().hdrFramebuffer.depthGuardBandThickness);

            // Left click: select by casting a ray through the center of the pixel
            float distance = finf();
            const shared_ptr<Entity>& newSelection = scene->intersect(ray, distance, m_sceneEditor->m_visualizationSettings.showMarkers,
                                                                      Array<shared_ptr<Entity>>(), m_sceneEditor->m_selectionInfo);                        
...
SceneEditorWindow::SceneEditorWindow(GApp* app, shared_ptr<Scene> scene, shared_ptr<GuiTheme> theme) : 
    ...
    m_app(app),
    m_selectionInfo(),
    m_sceneDropDownList(nullptr),
    m_cameraDropDownList(nullptr),
    m_showCameras(false),
    m_showLightSources(false),
    m_showLightBounds(false),
    m_frameEditor(nullptr),
...
void SceneEditorWindow::checkForChanges() {
    ...
    if (lastChange > m_lastStructuralChangeTime) {
        if (! m_sceneDropDownList->containsValue(m_scene->name())) {
            m_sceneDropDownList->append(m_scene->name());
        }
...
void SceneEditorWindow::makeGUI(GApp* app) {
    ...
    editorPane->beginRow(); {
        ...
        m_rateDropDownList = editorPane->addDropDownList("Rate", rates, nullptr, GuiControl::Callback(this, &SceneEditorWindow::onRateChange));
    ...
    {
        GuiPane* infoPane = tabPane->addTab("Info");
        infoPane->addLabel("Description")->moveBy(-4, 4);
        GuiTextBox* descBox = infoPane->addMultiLineTextBox("", Pointer<String>(m_scene, &Scene::description, &Scene::setDescription));
        descBox->setSize(290, 300);
    }
    ...
    // Debug visualizations
    {
        ...
        viewPane->beginRow(); {
            GuiControl* c = viewPane->addCheckBox("Axes", &m_visualizationSettings.showAxes);
            c->setWidth(w);
            c->moveBy(0, 4);
    ...
    // The entity editor
    {
        ...
        m_entityList = p->addDropDownList("Name", Array<String>(), nullptr, GuiControl::Callback(this, &SceneEditorWindow::onEntityDropDownAction));
        ...
        m_splineEditor->setVisible(false);
        m_popupSplineEditor = PhysicsFrameSplineEditor::create("Spline Editor", nullptr);
        ...
        m_perEntityPane->setSize(s_defaultWindowSize.x - 12, 400);
    ...
    // AO editor
    {
        ...
        // Show options that are useful for tuning quality or performance every few generations, but 
        // currently are not desirable to change
        const bool showAllOptions = false;
        ...
        m_aoPane->addNumberBox("Samples",             Pointer<int>(this, &SceneEditorWindow::aoNumSamples, &SceneEditorWindow::aoSetNumSamples), "", GuiTheme::LINEAR_SLIDER, 2, 99);
        ...
        m_aoPane->addCheckBox("Use Depth Peel Buffer", Pointer<bool>(this, &SceneEditorWindow::aoUseDepthPeelBuffer, &SceneEditorWindow::aoSetUseDepthPeelBuffer))->moveBy(0, 4);
        ...
        m_aoPane->addCheckBox("Temporal Filter", Pointer<bool>(this, &SceneEditorWindow::aoTemporallyVarySamples, &SceneEditorWindow::aoSetTemporallyVarySamples))->moveBy(0, 4);
    ...
    {
        GuiPane* selectedEntityPane = tabPane->addTab("Query");
        const float w = 275.0f;
        ...
        b = selectedEntityPane->addTextBox("Part", Pointer<String>(this, &SceneEditorWindow::selEntityPartName, &SceneEditorWindow::selEntitySetPartName)); b->setCaptionWidth(65);b->setWidth(w); b->setEnabled(true);
    ...
    tabPane->pack();
    tabPane->setWidth(s_defaultWindowSize.x + 16);
...
String SceneEditorWindow::selEntityPartName() const {
    if(notNull(m_selectionInfo.material)) {
        return m_selectionInfo.partName;
    }
    return "N/A";
}
...
void SceneEditorWindow::onPose(Array< shared_ptr< Surface > > &surfaceArray, Array< shared_ptr< Surface2D > > &surface2DArray) {
    ...
    if (m_showCameras) {
        ...
        for (int i = 0; i < name.length(); ++i) {
            const shared_ptr<Camera>& camera = m_scene->typedEntity<Camera>(name[i]);
            debugAssert(notNull(camera));
            // Don't visualize the active camera, because it just blocks the view
            if (camera != m_app->activeCamera()) {
                surfaceArray.append(VisualizeCameraSurface::create(camera));
            }
        }
    }
    ...
    if (m_visualizationSettings.showAxes) {
