/**
  \file G3D-app.lib/source/ArticulatedModel_ASSIMP.cpp
...
#include "../../external/assimp.lib/include/assimp/Importer.hpp"
#include "../../external/assimp.lib/include/assimp/scene.h"           // Output data structure
#include "../../external/assimp.lib/include/assimp/postprocess.h"     // Post processing flags
        ...
        for (unsigned int i = 0; i < aiPart->mNumMeshes; ++i) {
            ...
            if (mesh->HasFaces() && mesh->HasPositions()) {
                ...
                if ( materials.size() > 0 ) {
                    ...
                    g3dMesh->material = UniversalMaterial::createDiffuse(Color3::one() * 0.99f);
                }
...
public:
    void convert(ArticulatedModel* articulatedModel, aiNode* aiRootPart, 
        ...
        while ( !nodesToProcess.empty() ) {
            ...
            String partName;
            if ( currentNode->mParent == nullptr ) { // root
