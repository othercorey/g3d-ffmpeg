/**
  \file G3D-app.lib/source/SVO.cpp
...
SVO::SVO(const Specification& spec, const String& name, bool usebricks) :
    ...
    for (int f = 0; f < Field::COUNT; ++f) {
        ...
        if ((format != nullptr) && (f != Field::DEPTH_AND_STENCIL)) {
...
void SVO::init(RenderDevice* rd, size_t svoPoolSize, int maxTreeDepth, size_t fragmentPoolSize){
    ...
    if (isNull(m_rootIndex) || (m_rootIndex->size() < SVO_MAX_NUM_VOLUMES)) {
        ...
        m_rootIndex = BufferTexture::create("SVO::m_rootIndex", GLPixelTransferBuffer::create(SVO_MAX_NUM_VOLUMES * SVO_TOP_MIPMAP_NUM_LEVELS, 1, ImageFormat::R32UI(), nullptr, 1));
    }

    // Allocate Child pointer buffer
    if (isNull(m_childIndex) || (m_childIndex->size() < int(m_octreePoolNumNodes))) {
        // Allocate to the worst case size, which is the number of fragments
        m_childIndex = BufferTexture::create("SVO::childIndexBufferTexture", GLPixelTransferBuffer::create(m_octreePoolNumNodes, 1, ImageFormat::R32UI(), nullptr, 1));
    ...
    if ( isNull(m_parentIndex) || (m_parentIndex->size() < pnumnodes) ) {
        ...
        m_parentIndex = BufferTexture::create("SVO::parentIndexBufferTexture", GLPixelTransferBuffer::create(pnumnodes, 1, ImageFormat::R32UI(), nullptr, 1));
    ...
    if(m_useNeighborPointers && isNull(m_neighborsIndex)){
        ...
        m_neighborsIndex = BufferTexture::create("SVO::m_neighborsIndex", GLPixelTransferBuffer::create(width, pheight, ImageFormat::R32UI(), nullptr, depth));
...
void SVO::preBuild(RenderDevice* rd, bool multiPass, bool dummyPass){
    ...
    // Set the initial level to start at location 8 (we reserved location 0 to act as the nullptr pointer, and allocate in blocks of 8)
    ...
    // Zero the root's eight node pointers + the first 8 nullptr pointers
