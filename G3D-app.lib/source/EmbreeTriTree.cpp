/**
  \file G3D-app.lib/source/EmbreeTriTree.cpp
...
void EmbreeTriTree::apiConvert(const RTCRay& rtcRay, int triIndex, TriTree::Hit& hit) {
...
void EmbreeTriTree::apiConvertOcclusion(const RTCRay& rtcRay, TriTree::Hit& hit) {
    hit.triIndex = 0;
    hit.u = 0;
    hit.v = 0;
    hit.backface = false;        
...
void EmbreeTriTree::rebuild() {
    ...
    BEGIN_PROFILER_EVENT("EmbreeTriTree::rebuild");
    ...
    // Register the backface culler (it would theoretically be faster to set EMBREE_BACKFACE_CULLING at compile time and duplicate backfaces
    // for scenes that expect to have minimal two-sidedness, but the measured difference to the net path tracer is less than 1% performance)
    rtcSetOcclusionFilterFunctionN(m_scene, m_opaqueGeomID,    FilterAdapter::backfaceTest);
    rtcSetIntersectionFilterFunctionN(m_scene, m_opaqueGeomID, FilterAdapter::backfaceTest);
...
#       endif
        ...
        if (((adapter.options & TriTree::DO_NOT_CULL_BACKFACES) == 0) && (! tri.twoSided()) && (normal.dot(direction) >= 0.0f)) {
            ...
            const bool alphaTest = ((adapter.options & TriTree::NO_PARTIAL_COVERAGE_TEST) == 0);
            const float alphaThreshold = ((adapter.options & TriTree::PARTIAL_COVERAGE_THRESHOLD_ZERO) != 0) ? 1.0f : 0.5f;
...
void EmbreeTriTree::FilterAdapter::backfaceTest
(int*                        valid, 
 void*                       userDataPtr, 
 const RTCIntersectContext*  context,
 RTCRayN*                    ray,
 ...
 const size_t                N) {
    ...
    for (int r = 0; size_t(r) < N; ++r) {
        ...
        const Vector3   direction  (RTCRayN_dir_x(ray, N, r), RTCRayN_dir_y(ray, N, r), RTCRayN_dir_z(ray, N, r));
        const int       primID     = RTCHitN_primID(potentialHit, N, r);
        const Tri&      tri        = tree.m_triArray[tree.m_opaqueTriangleArray[primID].triIndex];
                
        if (((adapter.options & TriTree::DO_NOT_CULL_BACKFACES) == 0) && (! tri.twoSided()) && (normal.dot(direction) >= 0.0f)) {
            // Failed the backface test
            valid[r] = 0;
        } else {
            // Hit
            const float     u      = RTCHitN_u(potentialHit, N, r);
...
void EmbreeTriTree::intersectRays
   (const Array<Ray>&                  rays,
    ...
    if (isNull(m_scene)) { return; }
    BEGIN_PROFILER_EVENT("EmbreeTriTree::intersectRays");
