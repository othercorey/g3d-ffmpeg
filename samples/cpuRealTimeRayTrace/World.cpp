...
World::World() : m_mode(TRACE) {
    m_triTree = TriTree::create(false);
...
void World::insert(const shared_ptr<ArticulatedModel>& model, const CFrame& frame) {
    ...
    model->pose(posed, frame, CFrame(), nullptr, nullptr, nullptr, Surface::ExpressiveLightScatteringProperties());
...
shared_ptr<Surfel> World::intersect(const Ray& ray) const {
    ...
    m_triTree->intersectRay(ray, hit);
    m_triTree->sample(hit, surfel);
    return surfel;
}
