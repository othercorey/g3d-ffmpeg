...
layout(binding = 2) uniform SkyUniformBufferObject {
    vec4  light;
    vec4  resolution;
...
layout(location = 5) in flat float tangentYSign;
...
void main() {
    ...
    vec3 tangentX = normalize(inTangent.xyz);
    vec3 tangentY = normalize(cross(inNormal.xyz, inTangent.xyz)) * tangentYSign;

    vec3 baseColor = texture(texSampler, inTexCoord.xy).rgb;
    ...
    vec3 cameraPos = vec4(ubo.view[3]).xyz;
    vec3 view = normalize(cameraPos - inPosition.xyz);
    ...
    vec3 bsdf =
        ...
        //evaluateDisneyBRDF
        (baseColor, metallic, subsurface, specular, roughness, specularTint, anisotropic, sheen, sheenTint, clearcoat, clearcoatGloss, skyUBO.light.xyz, view, normal, tangentX, tangentY);
