...
int main(const int argc, const char* argv[]) {
    // 4.1 is supported on x64 and Tegra Windows, macOS, and Linux
    // 2.1 is the latest for Raspberry Pi 4 according to glxinfo
    const float desiredGLVersion = 4.1f;
    std::cout << "Minimal OpenGL " << desiredGLVersion << " Example by Morgan McGuire\n\nW, A, S, D, C, Z keys to translate\nMouse click and drag to rotate\nESC to quit\n\n";
    std::cout << std::fixed;
...
#   endif
    ...
    window = initOpenGL(windowWidth, windowHeight, fullScreen, vsync, "minimalOpenGL", desiredGLVersion);
