/**
  ...
  Distributed with the G3D Innovation Engine https://casual-effects.com/g3d
  ...
  All 3D math from https://graphicscodex.com
*/
#pragma once
#ifndef _MSC_VER 
...
GLFWwindow* initOpenGL(int width, int height, bool fullscreen, bool vsync, const std::string& title, float version = 4.1f) {
    if (! glfwInit()) {
    ...
    glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, int(version * 10) - int(version) * 10);
