...
void App::onGraphics3D(RenderDevice* rd, Array<shared_ptr<Surface> >& surface3D) {
    ...
    m_renderer->render(rd, activeCamera(), m_framebuffer, m_depthPeelFramebuffer, scene()->lightingEnvironment(), m_gbuffer, surface3D);
    ...
    rd->pushState(m_framebuffer); {
        ...
        model->pose(mySurfaces, manipulator->frame(), CFrame(), nullptr, nullptr, nullptr, Surface::ExpressiveLightScatteringProperties());
...
#   ifdef G3D_WINDOWS
      if (! FileSystem::exists("phong.pix", false) && FileSystem::exists("G3D.sln", false)) {
          ...
          // running with cwd = G3D-base/VC10/.  Change to
