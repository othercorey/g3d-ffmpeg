...
void App::initGBuffers() {
    ...
    m_gbufferSpecification.encoding[GBuffer::Field::SS_POSITION_CHANGE]    = 
    ...
    m_peeledGBufferSpecification.encoding[GBuffer::Field::SS_POSITION_CHANGE]    = nullptr;
    m_peeledGBufferSpecification.encoding[GBuffer::Field::SS_POSITION_CHANGE]    = nullptr;
    m_peeledGBufferSpecification.encoding[GBuffer::Field::EMISSIVE]              = nullptr;
    m_peeledGBufferSpecification.encoding[GBuffer::Field::GLOSSY]                = nullptr;
...
(RenderDevice*                   rd, 
 ...
 const shared_ptr<Texture>&      peeledDepthBuffer) {
    ...
    rd->push2D(fb); {
        ...
        const ProjInfo& projInfo = gbuffer->camera()->projection().reconstructFromDepthProjInfo(gbuffer->width(), gbuffer->height());
...
void App::computeGBuffers(RenderDevice* rd, Array<shared_ptr<Surface> >& all) {
    ...
    Surface::renderIntoGBuffer(rd, sortedVisible, m_peeledGBuffer);
    END_PROFILER_EVENT();
}
...
void App::evaluateDemoSettings() {
    ...
    case DemoSettings::CameraMode::STATIC:
        if (m_demoSettings.demoMode == DemoSettings::DemoMode::AO) {
            maybeSetActiveCamera(scene()->typedEntity<Camera>("staticAOCamera"));
        } else {
            maybeSetActiveCamera(scene()->typedEntity<Camera>("staticStatueCamera"));
        }
        break;

    case DemoSettings::CameraMode::DYNAMIC:
        if (m_demoSettings.demoMode == DemoSettings::DemoMode::AO) {
            maybeSetActiveCamera(scene()->typedEntity<Camera>("pillarStrafeCamera"));
        } else {
            maybeSetActiveCamera(scene()->typedEntity<Camera>("statuteStrafeCamera"));
        }
        break;
...
void App::maybeSetActiveCamera(const shared_ptr<Camera>& c) {
    if (notNull(c)) {
        setActiveCamera(c);
    }
}
