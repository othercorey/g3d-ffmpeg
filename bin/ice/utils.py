...
def _findWindowsBinary(program):
    ...
    # This function is often used to find MSBuild, 
    # so include the G3D Visual Studio path.
    G3D_VS_PATH = os.getenv('G3D_VS_PATH', '')

    # Add the path last. If the user has added the VS2017 MSBuild path
    # for backwards compatibility, buildg3d will not be able to find the
    # v142 toolset.
    PATH = [''] + \
           ['.',\
           '../bin/',\
           ...
           G3D_VS_PATH + '/Microsoft Visual Studio/2019/Enterprise/MSBuild/Current/Bin/',\
           G3D_VS_PATH + '/Microsoft Visual Studio/2019/Professional/MSBuild/Current/Bin/',\
           G3D_VS_PATH + '/Microsoft Visual Studio/2019/Community/MSBuild/Current/Bin/',\
           SYSTEMDRIVE + '/doxygen/bin/',\
           PROGRAMFILES + '/doxygen/bin/',\
           PROGRAMFILES + '/PKZIP/',\
           'bin/'] + os.getenv('PATH', '').split(';')
...
def MSBuild(filename, configs, targets = ['Debug', 'Release']):
    binary = _findWindowsBinary('MSBuild')
...
def VisualStudio(filename, configs, targets = ['Debug', 'Release']):
     ...
     if _findWindowsBinary('MSBuild'):
         return MSBuild(filename, configs, targets = targets)
     else:
...
def runWithOutput(prog, args = [], echo = True, env = None):
    ...
    if windows:
        program = _findWindowsBinary(program)
        if not program: raise Exception('Cannot find "' + str(prog) + '"')
