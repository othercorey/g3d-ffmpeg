/**
  \file G3D-base.lib/source/NetworkDevice.cpp
...
#include "G3D-base/networkHelpers.h"


namespace G3D {

NetworkDevice* NetworkDevice::s_instance = nullptr;
...
static int selectOneReadSocket(const SOCKET& sock) {
    ...
    int ret = select((int)sock + 1, &socketSet, nullptr, nullptr, &timeout);
...
static int selectOneWriteSocket(const SOCKET& sock) {
    ...
    return select((int)sock + 1, nullptr, &socketSet, nullptr, &timeout);
}
...
bool NetworkDevice::init() {
    ...
    // Winsock is initialized through the enet library
