/**
  \file G3D-base.lib/source/Projection.cpp
...
const char* FOVDirection::toString(int i, FOVDirection::Value& v) {
    static const char* str[] = {"HORIZONTAL", "VERTICAL", nullptr};
...
float Projection::nearPlaneViewportWidth(const Rect2D& viewport) const {
    ...
    float s = fabsf(2.0f * m_nearPlaneZ * tan(m_fieldOfView * 0.5f));
...
float Projection::imagePlanePixelsPerMeter(const class Rect2D& viewport) const {
    const float scale = fabsf(2.0f * tan(m_fieldOfView * 0.5f));
...
ProjInfo Projection::reconstructFromDepthProjInfo(int width, int height) const {
    Matrix4 P;
...
float Projection::pixelsPerMeter(float z, int width, int height) const {
    if (z >= 0.0f) {
        return finf(); 
    } else if (m_direction == FOVDirection::HORIZONTAL) {
        return float(width / (-2.0 * z * tan(m_fieldOfView * 0.5)));
    } else { // FOVDirection::VERTICAL
        return float(height / (-2.0 * z * tan(m_fieldOfView * 0.5)));
    }
}
