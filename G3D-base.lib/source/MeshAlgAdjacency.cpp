/**
  \file G3D-base.lib/source/MeshAlgAdjacency.cpp
...
public:
    ...
    private:
        ...
        Iterator(ET& a) : m_i0(-1), m_p(-1),  m_array(a), m_edgeArray(nullptr), m_end(false) {
            ++(*this);
        }
...
void MeshAlg::weldBoundaryEdges(
    ...
    for (int e = 0; e < oldEdgeArray.size(); ++e) {
        if (oldEdgeArray[e].boundary()) {
            ...
            const int v_low = min(oldEdgeArray[e].vertexIndex[0], oldEdgeArray[e].vertexIndex[1]);
