/**
  \file G3D-base.lib/source/Image.cpp
...
Image::~Image() {
    ...
    // This call can deinitialize the plugins if it is the last reference, but they can be re-initialized
...
shared_ptr<Image> Image::create(int width, int height, const ImageFormat* imageFormat) {
    alwaysAssertM(notNull(imageFormat), "imageFormat may not be ImageFormat::AUTO() or nullptr");
...
std::mutex Image::s_freeImageMutex;

void Image::initFreeImage() {
    std::lock_guard<std::mutex> guard(s_freeImageMutex);
...
bool Image::metaDataFromBinaryInput(BinaryInput& bi, int& width, int& height, const ImageFormat*& format) {
    initFreeImage();

    fipMemoryIO memoryIO(const_cast<uint8*>(bi.getCArray() + bi.getPosition()), static_cast<DWORD>(bi.getLength() - bi.getPosition()));
    fipImage image; 

    if (! image.loadFromMemory(memoryIO, FIF_LOAD_NOPIXELS)) {
        return false;
    }
...
shared_ptr<Image> Image::fromPixelTransferBuffer(const shared_ptr<PixelTransferBuffer>& buffer) {
    const shared_ptr<Image>& img = create(buffer->width(), buffer->height(), buffer->format());
    img->set(buffer);
    return img;
}
...
void Image::setSize(int w, int h, const ImageFormat* fmt) {
    if ((width() != w) ||
        ...
        alwaysAssertM(notNull(m_format), "Format may not be nullptr");
...
void Image::save(const String& filename) const {
    ...
    if (! m_image->save(filename.c_str(), flag)) {
        ...
        throw String("Image::save failed to write image to ") + filename.c_str();
    }
...
void Image::saveGIF(const String& filename, const Array<shared_ptr<Image>>& sequence, const double& fps) {
    if ((sequence.length() != 0) && (fps != 0.0f)) {
        ...
        for (int i = 0; i < sequence.length(); ++i) {
            ...
            FreeImage_SetMetadata(FIMD_ANIMATION, quantized, nullptr, nullptr);
...
shared_ptr<PixelTransferBuffer> Image::toPixelTransferBuffer() const {
    return toPixelTransferBuffer(Rect2D::xywh(0, 0, (float)width(), (float)height()));
}


shared_ptr<PixelTransferBuffer> Image::arrayToPixelTransferBuffer(const Array<shared_ptr<Image> >& images) {
    if (images.size() == 0) {
        return nullptr;
    }
...
shared_ptr<PixelTransferBuffer> Image::toPixelTransferBuffer(Rect2D rect, shared_ptr<PixelTransferBuffer> buffer) const {
...
shared_ptr<Image> Image::clone() const {
    const shared_ptr<Image>& c = createShared<Image>();
    *(c->m_image) = *m_image;
    c->m_format = m_format;
    return c;
}
...
bool Image::convertToL8() {
    if (m_image->convertToGrayscale()) {
        debugAssert(m_image->getBitsPerPixel() == 8);
