/**
  \file G3D-base.lib/source/CoordinateFrame.cpp
...
String CoordinateFrame::toXYZYPRDegreesString() const {
    ...
    return format("CFrame::fromXYZYPRDegrees(%.5g, %.5g, %.5g, %.5g, %.5g, %.5g)", 
...
//        *this = UprightFrame(any);
    } else if (beginsWith(n, "CFRAME::FROMXYZYPRRADIANS") || beginsWith(n, "COORDINATEFRAME::FROMXYZYPRRADIANS")) {
        any.verifyName("CFrame::fromXYZYPRRadians", "CoordinateFrame::fromXYZYPRRadians");
        any.verifyType(Any::ARRAY);
        any.verifySize(3, 6);

        const int s = any.size();

        *this = fromXYZYPRRadians(any[0], any[1], any[2], 
