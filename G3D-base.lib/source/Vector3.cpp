/**
  \file G3D-base.lib/source/Vector3.cpp

  \cite Portions based on Dave Eberly'x Magic Software Library at http://www.magic-software.com
...
Vector3::Vector3(const Vector4int8& v) :  x(v.x), y(v.y), z(v.z) {}

Vector3::Vector3(const Vector3uint8& v) : x(v.x), y(v.y), z(v.z) {}
...
void Vector3::cosPowHemiRandom(const Vector3& v, const float k, Random& r, Vector3& w, float& pdfValue) {
    ...
    pdfValue = powf(std::max(0.0f, v.dot(w)), std::max(k, 1e-6f)) * (1.0f + k) * (1.0f / (2.0f * pif()));
}
...
void Vector3::cosHemiPlusCosPowHemiHemiRandom(const Vector3& v, const Vector3& n, const float k, float P_cosPow, Random& rng, Vector3& w, float& pdfValue) {
    if (rng.uniform() < P_cosPow) {
        ...
        // Derivation:
        //
        //    pdfValue' = pdfValue * (1.0f - P_cosPow) + max(0.0f, v.dot(n)) * max(0.0f, dot(w, n)) * P_cosPow;
        //
