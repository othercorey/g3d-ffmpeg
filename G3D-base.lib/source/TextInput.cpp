/**
  \file G3D-base.lib/source/TextInput.cpp
...
numLabel:
    ...
    // Some unknown token
    throw ParseError(filename(), t.line(), t.character(), format("Unrecognized token type beginning with character '%c' (ASCII %d)", c, c));
    return;
}
