/**
  \file G3D-base.lib/source/Matrix3.cpp
...
bool Matrix3::is3DRotationDilation() const {
    const float L = row(0).squaredLength();
    return G3D::fuzzyEq(0.0f, row(0).dot(row(1))) &&
           G3D::fuzzyEq(0.0f, row(1).dot(row(2))) &&
           G3D::fuzzyEq(L, row(1).squaredLength()) &&
           G3D::fuzzyEq(L, row(2).squaredLength()) &&
           (row(0).cross(row(1)).dot(row(2)) > 0.0f) &&
           (L > 0.0f);
}
