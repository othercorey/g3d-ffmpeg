/**
  \file G3D-base.lib/source/fileutils.cpp
...
String readWholeFile(const String& filename) {
    ...
    if (! FileSystem::inZipfile(filename, zipfile, internalFile)) {
        // Not in zipfile
        if (! FileSystem::exists(filename)) {
...
static bool _zip_zipContains(const String& zipDir, const String& desiredFile){
    ...
    zip_int64_t test = zip_name_locate( z, desiredFile.c_str(), ZIP_FL_NOCASE );
    zip_close( z );
    if(test == -1){
        return false;
    }
...
String generateFileNameBaseAnySuffix(const String& prefix) {
    Array<String> exist;
    const String& path = FilePath::parent(prefix);
    const String& base = FilePath::base(prefix);
    String templat = FilePath::concat(path, System::currentDateString()) + "_" + base;
    FileSystem::getFiles(templat + "*", exist, true);
    if (exist.size() == 0) {
...
String filenamePath(const String& filename) {
    ...
    if (i == String::npos || ((j > i) && (j != String::npos))) {
        i = j;
    }
