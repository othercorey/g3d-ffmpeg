/**
  \file G3D-base.lib/source/AABox.cpp
...
void AABox::getTransformedBounds(const class Matrix3x4& xform, AABox& bounds) const {
    bounds = AABox(xform * Vector4(corner(0), 1.0f));

    for (int i = 1; i < 8; ++i) {
        bounds.merge(xform * Vector4(corner(0), 1.0f));
    }
}
...
AABox AABox::minkowskiSum(const AABox& other) const {
    if (isEmpty()) {
        ...
        return AABox(low() - (other.center() - other.low()), high() + (other.high() - other.center()));
    }
}
