/**
  \file G3D-base.lib/source/Matrix.cpp
...
Matrix::Impl::Impl(const Matrix3& M) : elt(nullptr), data(nullptr), R(0), C(0), dataSize(0){
    setSize(3, 3);
    for (int r = 0; r < 3; ++r) {
        for (int c = 0; c < 3; ++c) {
            set(r, c, M[r][c]);
...
Matrix::Impl::Impl(const Matrix4& M): elt(nullptr), data(nullptr), R(0), C(0), dataSize(0) {
    setSize(4, 4);
    for (int r = 0; r < 4; ++r) {
        for (int c = 0; c < 4; ++c) {
            set(r, c, M[r][c]);
