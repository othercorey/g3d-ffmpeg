/**
  \file G3D-base.lib/source/MeshAlg.cpp
...
void MeshAlg::identifyFeatureEdges
   (const Array<Vector3>&           vertexArray,
    ...
    // If the dot product of adjacent face normals is
    // less than this (which might be negative!), then the edge
    // between those faces is a crease.
    const float creaseThreshold = cos(pif() - creaseAngle);

    feature.resize(edgeArray.size());
    for (int e = 0; e < edgeArray.size(); ++e) {
        const Edge& edge = edgeArray[e];
        if ((edge.faceIndex[0] == Face::NONE) || (edge.faceIndex[1] == Face::NONE)) {
            // Border
            feature[e] = true;
        } else {
            const float cos = faceNormalArray[edge.faceIndex[0]].dot(faceNormalArray[edge.faceIndex[1]]);
            feature[e] = cos <= creaseThreshold;
        }
    }
}
