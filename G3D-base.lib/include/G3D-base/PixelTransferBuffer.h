/**
  \file G3D-base.lib/include/G3D-base/PixelTransferBuffer.h
...
*/
#pragma once
#define G3D_base_PixelTransferBuffer_h
...
public:
    ...
    /** Return the byte offset from the mapped pointer for the row to raw pixel data at start of row \a y of depth \a d. */
    size_t rowOffset(int y, int d = 0) const {
        debugAssert(y < m_height && d < m_depth);
        return (d * m_height * m_rowStride) + (y * m_rowStride); 
    }
    ...
    */
    ...
    /** If true, the data lifetime is independent of the PixelTransferBuffer lifetime. For example,
        holding the PixelTransferBuffer may not keep the data valid if ownsMemory() = false. */
...
public:
    static shared_ptr<BufferUnmapper> create(const shared_ptr<PixelTransferBuffer>& b) { return createShared<BufferUnmapper>(b); }
    virtual void* alloc(size_t s) override { debugAssert(false); return nullptr; }
