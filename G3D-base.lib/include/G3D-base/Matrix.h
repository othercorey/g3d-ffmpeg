/**
  \file G3D-base.lib/include/G3D-base/Matrix.h
...
public:
    ...
    private:
        ...
        inline Impl() : elt(nullptr), data(nullptr), R(0), C(0), dataSize(0) {}

        Impl(const Matrix3& M);

        Impl(const Matrix4& M);

        inline Impl(int r, int c) : elt(nullptr), data(nullptr), R(0), C(0), dataSize(0) {
            setSize(r, c);
        }

        Impl& operator=(const Impl& m);

        inline Impl(const Impl& B) : elt(nullptr), data(nullptr), R(0), C(0), dataSize(0) {
...
#   undef DECLARE_METHODS_1
    ...
    /**
      ...
      @return nullptr on success, a string describing the error on failure.
