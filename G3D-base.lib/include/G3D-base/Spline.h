/**
  \file G3D-base.lib/include/G3D-base/Spline.h
...
public:
    ...
    /** If SplineInterpolationMode::CYCLIC, then the control points will be assumed to wrap around.
        If SplineInterpolationMode::LINEAR, then the tangents at the ends of the spline
        point to the final control points. If SplineInterpolationMode::CLAMP, the end control 
    ...
    */
    ...
    /** For a cyclic spline, this is the time elapsed between the last
        control point and the first. If less than zero this is
        assumed to be:
...
public:
    ...
    */
    void getControl(int i, float& t, Control& c) const {
        // Number of control points
        const int N = control.size();

        if (N == 0) {
            // Special case: No control points (empty spline)
            c = zero;
            t = 0;
            ...
            if (i < 0) {
                ...
                const int wraps = (N + 1 - i) / N;                    
                const int j = (i + wraps * N) % N;
            ...
            if (N >= 2) {
                const float dt = time[N - 1] - time[N - 2];
