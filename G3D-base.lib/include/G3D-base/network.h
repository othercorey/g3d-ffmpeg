/**
  \file G3D-base.lib/include/G3D-base/network.h
...
/**
    ...
    This is a way to send out-of-band information, such as transferring a giant file on
...
/**
  If NOT using G3D's internal threaded networking, you must invoke this periodically to service
  the receiving side network connections.  Receiving messages will only update
...
/**
  If NOT using G3D's internal threaded networking, you must invoke this periodically to service
  the sending side network connections. Sending messages will only update
  inside this call--all other network calls queue for processing.
...
public:
    ...
    */
    ...
    NetMessageIterator& begin() {
        return isValid() ? *this : s_doneIterator;
    }
    ...
    /** Advance to the next message and deallocate the object referenced by 
        binaryInput. Returns end() when the iterator is empty. */
...
public:
    ...
    static shared_ptr<NetServer> create
        (uint16                            listenPort,
         ...
         size_t                            incomingBytesPerSecondThrottle = UNLIMITED_BANDWIDTH,
         size_t                            outgoingBytesPerSecondThrottle = UNLIMITED_BANDWIDTH) {
        return create(NetAddress(NetAddress::localHostname(), listenPort), maxClients, numChannels, incomingBytesPerSecondThrottle, outgoingBytesPerSecondThrottle); 
    }
...
public:
    ...
    NetworkCallbackInfo() : data(nullptr) {}
...
protected:
    ...
    ChannelMessageIteratorTable     m_netMessageIteratorTable; // one iterator per channel
...
public:
    ...
    */
    ...
    /** Check the network for new messages and return an iterator over them.
        For same channels this returns the same iterator.
        If no iterator is available on the channel the return value is a null pointer
        */
    NetMessageIterator& incomingMessageIterator(const NetChannel &channel);
    ...
    const Array<NetChannel> getIncomingChannels() const;
