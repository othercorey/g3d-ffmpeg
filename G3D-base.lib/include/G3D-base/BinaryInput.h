/**
  \file G3D-base.lib/include/G3D-base/BinaryInput.h
...
private:
    ...
    // may grow if a large memory read occurs.  5 GB
    static const int64
        INITIAL_BUFFER_LENGTH = 5000000000L  // 5 GB
    ;
...
public:
   ...
   float64 readFloat64() {
    ...
    /**
     Always consumes \a maxLength characters.  Reads a string until nullptr or \a maxLength characters. Does not require nullptr termination.
     */
    ...
    /** Read a string (which may contain NULLs) of exactly numBytes bytes, including the final terminator if there is one.  If there is a nullptr in the string before
        the end, then only the part up to the first nullptr is returned although all bytes are read.*/      
    String readFixedLengthString(int numBytes);

    /** 
     Reads a string until nullptr, newline ("&#92;r", "&#92;n", "&#92;r&#92;n", "&#92;n&#92;r") or the end of the file is encountered. Consumes the newline.
     */
    String readStringNewline();

    /**
     Reads until nullptr or the end of the file is encountered.
     If the string has odd length (including nullptr), reads 
