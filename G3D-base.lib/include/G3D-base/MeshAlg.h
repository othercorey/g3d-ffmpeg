...
public:
    ...
    */
    ...
    /**
      Flags border (only one face, edge of the mesh), ridge (sharp convex),
      and valley (sharp concave) edges.

      \param creaseAngle Interior or exterior angles sharper than this (in radians) between
        adjacent faces are considered creases (corners). Default is 120 degrees, which is 
        slightly wider than the angles in a dodecahedron.
     */
    static void identifyFeatureEdges
       (const Array<Vector3>&           vertexArray,
