/**
  \file G3D-base.lib/include/G3D-base/Projection.h
...
class FOVDirection {
public:
    enum Value {HORIZONTAL, VERTICAL, DIAGONAL} value;
...
private:
    ...
    /** Full field of view (in radians) in direction m_direction */
    float                       m_fieldOfView;
...
public:
    ...
    */
    ...
    /** Returns the scale factor between a length orthogonal to the viewing axis
        and screen pixels, at a given (negative) distance \a z along the viewing 
        axis.

        For example, this is the scale factor to multiply the radius of a viewer-aligned
        disk by to obtain its pixel radius under perspective projection.

        \param z must be negative
      */
