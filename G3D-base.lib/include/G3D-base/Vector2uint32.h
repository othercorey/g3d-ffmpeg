/**
  \file G3D-base.lib/include/G3D-base/Vector2uint32.h
...
public:
    ...
    static Vector2uint32 parseResolution(const String& s);
    ...
    Vector2uint32 max(const Vector2uint32& v) const {
        return Vector2uint32(G3D::max(x, v.x), G3D::max(y, v.y));
    }

    Vector2uint32 min(const Vector2uint32& v) const {
        return Vector2uint32(G3D::min(x, v.x), G3D::min(y, v.y));
    }
