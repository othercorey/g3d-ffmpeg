/**
  \file G3D-base.lib/include/G3D-base/Array.h
...
public:
   ...
   explicit Array(const std::vector<T>& other) : num(0), data(nullptr) {
       *this = other;
   }
    ...
    */
    ...
    void append(const T& v1, const T& v2, const T& v3, const T& v4, const T& v5, const T& v6, const T& v7) {
        if (inArray(&v1) || inArray(&v2) || inArray(&v3) || inArray(&v4) || inArray(&v5) || inArray(&v6) || inArray(&v7)) {
            T t1 = v1;
            T t2 = v2;
            T t3 = v3;
            T t4 = v4;
            ...
            T t6 = v6;
            T t7 = v7;
            append(t1, t2, t3, t4, t5, t6, t7);
        } else if (num + 7 < numAllocated) {
            // This is a simple situation; just stick it in the next free slot using
            // the copy constructor.
            new (data + num) T(v1);
            new (data + num + 1) T(v2);
            ...
            resize(num + 7, DONT_SHRINK_UNDERLYING_ARRAY);
            data[num - 7] = v1;
            data[num - 6] = v2;
            data[num - 5] = v3;
            data[num - 4] = v4;
