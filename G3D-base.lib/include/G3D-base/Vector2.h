/**
  \file G3D-base.lib/include/G3D-base/Vector2.h
...
public:
    ...
    static Vector2 hammersleySequence2D(int rayIndex, int raysPerPixel) {
        Vector2 P;
        G3D::hammersleySequence2D(rayIndex, raysPerPixel, P.x, P.y);
        return P;
    }

    /** \sa mod1() */
    Vector2 wrap(const Vector2& v) const {
        return Vector2(G3D::wrap(x, v.x), G3D::wrap(y, v.y));
    }

    /** \sa mod1() */
    Vector2 wrap(float f) const {
        return wrap(Vector2(f, f));
    }

    /** Fast implementation of wrap(1.0f) */
    Vector2 mod1() const {
        return Vector2(G3D::mod1(x), G3D::mod1(y));
    }
