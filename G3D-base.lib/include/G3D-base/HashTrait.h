/**
  \file G3D-base.lib/include/G3D-base/HashTrait.h
...
template <typename T> struct HashTrait<std::shared_ptr<T>> {
    static size_t hashCode(const std::shared_ptr<T>& k) { return reinterpret_cast<size_t>(k.get()) >> 1; }
};
