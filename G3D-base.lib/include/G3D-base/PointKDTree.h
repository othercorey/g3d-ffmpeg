/**
  \file G3D-base.lib/include/G3D-base/PointKDTree.h
...
#include "G3D-base/PositionTrait.h"
#include <algorithm>
...
public:
    ...
    /** To construct a balanced tree, insert the elements and then call
      PointKDTree::balance(). */
    PointKDTree() : root(nullptr) {}


    PointKDTree(const PointKDTree& src) : root(nullptr) {
        *this = src;
    }
    ...
    static void getIntersectingMembers(
        ...
        if (parentMask == 0) {
            ...
            if (node->valueArray.size() >  0) {
                ...
                debugAssertM(node->child[0] == nullptr, "Malformed Point tree");
                debugAssertM(node->child[1] == nullptr, "Malformed Point tree");
