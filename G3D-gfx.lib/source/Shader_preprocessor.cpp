/**
  \file G3D-gfx.lib/source/Shader_preprocessor.cpp
...
bool Shader::processIncludes(const String& dir, String& code, String& messages, const Args& args) {
    ...
    do {
        ...
        // Find the next #include or #eval, whichever is first
        const size_t iLoc = findPragmaWithSpaces("include", code, includeLoc);
        const size_t eLoc = findPragmaWithSpaces("eval", code, includeLoc);
        ...
        bool macroArgumentNotBound = false;
        if (includeLoc != String::npos) {
            ...
            String includedFilenameOrVariable;
            TextInput t (TextInput::FROM_STRING, includeLine);

            if (isEval) {
                t.readSymbols("#", "eval", "(");
                includedFilenameOrVariable = t.readSymbol();
                t.readSymbol(")");
                ...
                if (t.peek().extendedType() == Token::SYMBOL_TYPE) {
                    ...
                    const String s = t.readSymbol();

                    if (s == "<") { 
                        // System filename
                        includedFilenameOrVariable = t.readUntilDelimiterAsString('>');
                        t.readSymbol(">");
                        includedFilenameOrVariable = System::findDataFile(includedFilenameOrVariable);
                    } else {
                        // Macro argument containing the filename
                        if (! args.getMacro(s, includedFilenameOrVariable)) {
                            // The macro was not bound. We cannot create an error because this is 
                            // executing before #ifdef, so we don't know if that #include was supposed
                            // to be processed. Fail silently in this case and insert nothing.
                            macroArgumentNotBound = true;
                            includedFilenameOrVariable = s;
                        }
                    }

                } else {
                    // Regular #include relative to the current file
                    debugAssert(t.peek().extendedType() == Token::DOUBLE_QUOTED_TYPE);

                    includedFilenameOrVariable = t.readString();
                    includedFilenameOrVariable = FilePath::canonicalize(includedFilenameOrVariable);

                    // Resolve non-absolute paths relative to the current file
                    if (! beginsWith(includedFilenameOrVariable, "/")) {
                        includedFilenameOrVariable = FilePath::concat(dir, includedFilenameOrVariable);
                    }
                }
            } // end parsing
            ...
            if (isEval) {
                if (args.getMacro(includedFilenameOrVariable, includedValue)) {
                } else {
                    ok = false;
                    messages += format("%s(%d): Macro argument not bound for #eval: %s.\n", 
                                        lastFile.c_str(), lastLineNumber + linesSinceLastLineNumber,
                                        includedFilenameOrVariable.c_str());
                }
            } else {
                // #include
                try {
                    bool created = false;
                    String& contents = includedFileContentsCache.getCreate(includedFilenameOrVariable, created);
                     
                    if (created) {
                        contents = macroArgumentNotBound ? 
                            format("#error Macro used in shader but not bound to Args at runtime in \"#include %s\"\n", includedFilenameOrVariable.c_str()) : 
                            readWholeFile(includedFilenameOrVariable);

                        includedValue = contents;
            ...
            if (! endsWith(includedValue, "\n")) {
                includedValue += "\n";
            }

            code = code.substr(0, includeLoc) + getLinePragma(1, includedFilenameOrVariable) + includedValue +
...
bool Shader::g3dLoadTimePreprocessor(const String& dir, PreprocessedShaderSource& source, String& messages, GLuint stage, const Args& args) {
    ...
    // Handle #include and #eval directives first, since they may affect
    // what preprocessing is needed in the code. 
    code = getLinePragma(1, name) + code;
    bool ok = processIncludes(dir, code, messages, args);     
    ...
    if (ok) {
        ...
        String uniformString = 
            STR(uniform highp mat4x3 g3d_WorldToObjectMatrix;
                uniform highp mat4x3 g3d_ObjectToWorldMatrix;
                uniform highp mat4   g3d_ProjectionMatrix;
                uniform highp mat4x4 g3d_ProjectToPixelMatrix;
                uniform highp mat3   g3d_WorldToObjectNormalMatrix;
                uniform highp mat3   g3d_ObjectToWorldNormalMatrix;
                uniform highp mat3   g3d_ObjectToCameraNormalMatrix;
                uniform highp mat4x3 g3d_ObjectToCameraMatrix;
                uniform highp mat3   g3d_CameraToObjectNormalMatrix;
                uniform highp mat4x3 g3d_WorldToCameraMatrix;
                uniform highp mat4x3 g3d_CameraToWorldMatrix;
                uniform highp vec4   g3d_ProjInfo;
                uniform highp vec3   g3d_ClipInfo;
                uniform highp float  g3d_SceneTime;
                uniform highp float  g3d_RunTime;
                uniform bool g3d_InvertY;
                uniform highp mat3 g3d_WorldToCameraNormalMatrix;
                uniform highp mat4 g3d_ObjectToScreenMatrixTranspose;
                uniform highp mat4 g3d_ObjectToScreenMatrix;
                uniform highp vec2 g3d_FragCoordExtent;
                uniform highp vec2 g3d_FragCoordMin;
                uniform highp vec2 g3d_FragCoordMax;
                uniform highp vec2 g3d_FramebufferSize;
                uniform highp int g3d_NumInstances;
...
static bool canonicalizeVersionLine(String& versionLine) {
    ...
    validGLSLVersions.append(310, 320, 330, 400, 410, 420, 430);
    validGLSLVersions.append(440, 450, 460);

    const int highestVersionSupportedOnThisGPU = iRound(GLCaps::glslVersion() * 100);

    versionLine = trimWhitespace(versionLine);
    if (endsWith(versionLine, " es")) {
        // Do not process GLES shaders further
        return true;
    }
    ...
    int version = -1;
    int nextVersion = -1;
    String chosenPhrase = GLCaps::api() == OSWindow::Settings::API_OPENGL_ES ? "version 310 es" : "version 410";//default version line
    String lastPhrase = "";
    ...
    while (ti.hasMore()) {
        ...
        if (t.string() == "or") { 
            ...
            //append other tokens (like "compatability", "es",  or the optional word "version") to the current version phrase
            lastPhrase += t.string() + " ";
        }
    }
...
void Shader::processExtensions(String& code, String& extensionLines) {
    ...
    while (i != String::npos) {
        ...
        // Remove the current line from the shader
        size_t end = code.find("\n", i + 1);
        if (end == String::npos) {
            end = code.size();
        }
