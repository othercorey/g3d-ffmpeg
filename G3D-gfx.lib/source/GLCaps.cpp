/**
  \file G3D-gfx.lib/source/GLCaps.cpp
...
float GLCaps::m_glVersionNumber = 0;

bool GLCaps::m_hasTexelFetch = false;

float GLCaps::m_glslVersion = 0.0f;

OSWindow::Settings::API GLCaps::s_api = OSWindow::Settings::API_OPENGL;

/**
 Dummy function to which unloaded extensions can be set.
 */
static void glIgnore(GLenum e) {
    (void)e;
}
...
const ImageFormat* GLCaps::smallHDRFormat() {
    ...
    if (!f) {
        Array<const ImageFormat*> preferredColorFormats = { 
            ImageFormat::R11G11B10F(), ImageFormat::RGB16F(), ImageFormat::RGBA16F(), 
...
GLCaps::Vendor GLCaps::computeVendor() {
    ...
    if (! initialized) {
        const String& s = vendor();
        const String& _glVersion = (char*)glGetString(GL_VERSION);

        if ((s == "ATI Technologies Inc.") || (s == "X.Org")) {
            v = AMD;
        } else if ((beginsWith(s, "Intel")) || (s == "Intel Open Source Technology Center")) {
            v = INTEL;
        } else if (beginsWith(s, "NVIDIA")) {
            v = NVIDIA;
...
void GLCaps::init(OSWindow::Settings::API api) {
    s_api = api;

    debugAssertGLOk();
    const char* v = (const char*)glGetString(GL_SHADING_LANGUAGE_VERSION);
    GLenum e = glGetError();
    if ((v != nullptr) && (e == GL_NONE)) {
    ...
    if (s_api != OSWindow::Settings::API_OPENGL_ES) {
...
void GLCaps::loadExtensions(Log* debugLog) {
    ...
    debugAssert(glGetString(GL_VENDOR) != nullptr);
    ...
    sscanf(glVersion().c_str(), "%f", &m_glVersionNumber);
    ...
    m_glMajorVersion = int(m_glVersionNumber);
...
#       undef DECLARE_EXT
    ...
    if (! GLCaps::supports_GL_ARB_multitexture()) {
        ...
        m_numTextureCoords = max(1, m_numTextureCoords);
        m_numTextures      = max(1, m_numTextures);
        m_numTextureUnits  = max(1, m_numTextureUnits);
    }
...
bool GLCaps::supportsTexture(const ImageFormat* fmt) {
    ...
    if (! _supportedImageFormat().containsKey(fmt)) {
        ...
        // NSight under windows complains about the glGetInternalformati64v call.
        if (supports("GL_ARB_internalformat_query2")) {
            // GL 4.0 style check:
            int64 result = 0;
            debugAssertGLOk();
        ...
        if ( (! supports_GL_ARB_framebuffer_object() && ! supports_GL_EXT_framebuffer_object()) // No frame buffers
            ...
            if (true) {
                ...
                glPushAttrib(GL_COLOR_BUFFER_BIT); {
                    ...
                    glTexImage2D(GL_TEXTURE_2D, mipLevel, fmt->openGLFormat, width, height, border, fmt->openGLBaseFormat, GL_UNSIGNED_BYTE, nullptr);
...
#       define REQUIRE(ext, alt) \
        { bool has = supports(ext) || supports(alt);\
            explanation += format("%33s  %s\n", ext, (has ? " yes " : " NO - Required for G3D 10.0!"));\
...
#       ifdef G3D_WINDOWS
            if (wglGetProcAddress("wglCreateContextAttribsARB") == nullptr) {
...
float GLCaps::glVersionNumber() {
    return m_glVersionNumber;
}

bool GLCaps::supports_glTexStorage2D() {
    //static bool result = glVersionNumber() >= 4.2f;;
    return false;//result;
}
