/**
  \file G3D-gfx.lib/source/GLFWWindow.cpp
...
#   if defined(G3D_OSX)
#       include "G3D-gfx/CocoaInterface.h"
#   endif
...
float GLFWWindow::defaultGuiPixelScale() {
    #ifdef G3D_OSX
    // Our OSX framebuffer is already taking the pixel scale
    // into account because Macs have such high resolutions
    return 1.0f;
    #endif
...
static void windowCloseCallback( GLFWwindow* w ){
    //TODO: currently cannot close the second window
    if (GLFWWindow::getMasterWindowPtr() == w) {
...
Vector2 GLFWWindow::virtualDisplaySize() {
    ...
    for (int i = 0; i < monitorCount; ++i) {
        ...
        maxDisplayCoordinate.x = max(maxDisplayCoordinate.x, (float)monitorPosX + vidMode->width);
        maxDisplayCoordinate.y = max(maxDisplayCoordinate.y, (float)monitorPosY + vidMode->height);
        minDisplayCoordinate.x = min(minDisplayCoordinate.x, (float)monitorPosX);
        minDisplayCoordinate.y = min(minDisplayCoordinate.y, (float)monitorPosY);
    }
...
#   ifdef G3D_WINDOWS
    {
        // Checking the display size first triggers a non-high DPI scaled result for SPI_GETWORKAREA
        const Vector2 displaySize((float)GetSystemMetrics(SM_CXSCREEN), (float)GetSystemMetrics(SM_CYSCREEN));
        (void)displaySize;
        RECT workarea;
...
#   else
        const Vector2 v = primaryDisplaySize();
...
float GLFWWindow::primaryDisplayRefreshRate(int width, int height) {
    ...
    for (int i = 0; i < count; ++i) {
        if (modes[i].width >= width && modes[i].height >= height) {
            maxRefresh = max(maxRefresh, float(modes[i].refreshRate));
        }
    }
...
static GLFWvidmode findClosestVidMode(const GLFWvidmode* modes, int count, const OSWindow::Settings& settings) {
    ...
    // Find the closest height, remove all that do not fit
    int targetHeight = settings.height;
    int minHeightDistance = 100000;
    for (const GLFWvidmode& mode : vidModes) {
    ...
    if (settings.refreshRate <= 0) {
        // Find the highest refresh supported by this monitor
        for (const GLFWvidmode& mode : vidModes) {
            targetRefreshRate = max(targetRefreshRate, mode.refreshRate);
        }
    } else {
        minRefreshRateDist = 10000;
        for (const GLFWvidmode& mode : vidModes) {
...
GLFWwindow* GLFWWindow::s_share = nullptr;
bool        GLFWWindow::s_inputCapture = false;
Array<int>  GLFWWindow::s_joystickMapping;

GLFWWindow::GLFWWindow(const OSWindow::Settings& settings) : m_currentKeyMod(GKeyMod::NONE) {

    // This code can run before debugAssertGLOk() can run
#   define LOW_LEVEL_GL_ERROR_CHECK() {\
        const GLenum e = glGetError();\
        if (e != GL_NO_ERROR) {\
            debugPrintf("OpenGL error during initialization: %s\n", GLenumToString(e));\
        }\
    ...
    if (isNull(OSWindow::current())) {
        setCurrentWindowPtr(this);
    }
    m_mouseVisible = true;
    s_inputCapture = false;
    m_iconified    = false;
    m_settings = settings;

    glfwWindowHint(GLFW_COCOA_RETINA_FRAMEBUFFER, GLFW_FALSE);

    if (settings.api == Settings::API_OPENGL_ES) {
    ...
    for (int i = 0; i < monitorCount; ++i) {
        ...
        if (name == settings.fullScreenMonitorName) {
            monitor = monitors[i];
        }
    }
    
    if (! settings.fullScreenMonitorName.empty() &&
        (settings.fullScreenMonitorName != glfwGetMonitorName(monitor))) {
        debugPrintf("Unabled to find monitor %s; selected primary monitor %s\n", settings.fullScreenMonitorName.c_str(), glfwGetMonitorName(monitor));
    }

    monitor = settings.fullScreen ? monitor : nullptr;
...
#       endif
        ...
        //debugPrintf("Selecting width = %d, height = %d, refresh = %d\n", mode.width, mode.height, mode.refreshRate);
    ...
    m_glfwWindow = glfwCreateWindow(width, height, settings.caption.c_str(), monitor, s_share);

    if (isNull(s_share)) {
        s_share = m_glfwWindow;
    }

    if (isNull(m_glfwWindow)) {
        glfwTerminate();
        if (s_share == m_glfwWindow) {
            s_share = nullptr;
        }
...
#   undef LOW_LEVEL_GL_ERROR_CHECK
}
...
String GLFWWindow::joystickName(unsigned int stickNum) const {
    ...
    return glfwGetJoystickName(s_joystickMapping[stickNum]);
}
...
#   ifdef G3D_WINDOWS
        ...
        for (int y = 0; y < src->height(); ++y) {
            for (int x = 0; x < src->width(); ++x) {
                ...
                uint8 bit = (pixel.a > 127 ? 1 : 0) << (x & 7);
                binaryMaskData[(y * (src->width() >> 3)) + (x >> 3)] |= bit;
            }
        }
        const shared_ptr<PixelTransferBuffer>& bgraColorBuffer = ImageConvert::convertBuffer(colorData->toPixelTransferBuffer(), ImageFormat::BGRA8());
...
void GLFWWindow::getJoystickState(unsigned int stickNum, Array<float> &axis, Array<bool> &buttons) const {
    alwaysAssertM(numJoysticks() > int(stickNum), format("Tried to access joystick %d, only %d available.", stickNum, numJoysticks()));
    const int mappedStickNum = s_joystickMapping[stickNum];
...
int GLFWWindow::visibleCursorMode() {
    return s_inputCapture ? GLFW_CURSOR_DISABLED : GLFW_CURSOR_NORMAL;
}
...
void GLFWWindow::setAsCurrentGraphicsContext() const {
    glfwMakeContextCurrent(m_glfwWindow);
}
