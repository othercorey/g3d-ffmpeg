/**
  \file G3D-gfx.lib/source/tesselate.cpp
...
static  void _tesselateBegin(GLenum e, TessData* data) {
    if (e == GL_TRIANGLES || e == GL_TRIANGLE_FAN || e == GL_TRIANGLE_STRIP) {
        data->primitive.append(TessData::Primitive(e));
    }
}

static void _tesselateVertex(Vector3* v, TessData* data) {
    data->primitive.last().vertex.append(*v);
}

static void _tesselateEnd(TessData* data) {
    (void)data;
}
...
static void _tesselateError(GLenum e) {
...
void tesselateComplexPolygon(const Array<Vector3>& input, Array<Triangle>& output) {
    ...
    static GLUtriangulatorObj* tobj = nullptr;
