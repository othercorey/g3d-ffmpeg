...
Texture::Encoding Texture::Encoding::lowPrecisionScreenSpaceMotionVector() {
    if (GLCaps::supportsTexture(ImageFormat::RG8_SNORM())) {
        return Texture::Encoding(ImageFormat::RG8_SNORM(), FrameName::SCREEN, 64.0f, 0.0f);
    } else {
        ...
        const float range = 16320.0f / 127.0f;      // Range value just under 128
        return Texture::Encoding(GLCaps::supportsTexture(ImageFormat::RG8()) ?
            ImageFormat::RG8() :
            ImageFormat::RGBA8(),
            FrameName::SCREEN, range, -range / 2.0f);
    }
}
...
shared_ptr<Texture> Texture::uniformRandom() {
    ...
    if (isNull(t)) {
        Random rnd;
        const shared_ptr<GLPixelTransferBuffer>& ptb = GLPixelTransferBuffer::create(1024, 1024, ImageFormat::RG16_SNORM());
        {
        ...
        t = Texture::fromPixelTransferBuffer("G3D::Texture::uniformRandom", ptb, ImageFormat::RG16_SNORM(), Texture::DIM_2D, false);
        t->visualization.max = 1.0f;
        ...
        glStatePush();
        {
            ...
            for (int t = 0; t < targets.size(); ++t) {
                glTexImage3D(targets[t], 0, format()->openGLFormat, w, h, d,
                                     0, format()->openGLBaseFormat, GL_UNSIGNED_BYTE, nullptr);
            }
        }
        glStatePop();
...
static void setMipBias(GLenum target, float mipBias) {
    if ((mipBias != 0.0f) && (GLCaps::api() == OSWindow::Settings::API_OPENGL)) {
...
shared_ptr<GLPixelTransferBuffer> Texture::toPixelTransferBuffer(const ImageFormat* outFormat, int mipLevel, CubeFace face) const {
    ...
    //
    // the pixel transfer buffer size is derived from texture size and request mipmap level with size at least size of 1x1 pixels
    // 
    //shared_ptr<GLPixelTransferBuffer> buffer = GLPixelTransferBuffer::create(width() >> mipLevel, height() >> mipLevel, outFormat, nullptr, mipDepth, GL_STATIC_READ); // throws error for width or height == 0
    shared_ptr<GLPixelTransferBuffer> buffer = GLPixelTransferBuffer::create(std::max(1, width() >> mipLevel), std::max(1, height() >> mipLevel), outFormat, nullptr, mipDepth, GL_STATIC_READ);
    END_PROFILER_EVENT();
...
void Texture::setShaderArgs(UniformTable& args, const String& prefix, const Sampler& sampler) {
    ...
    debugAssertM((void*)this != nullptr, "Memory corruption");
...
CUarray &Texture::cudaMap(unsigned int  usageflags){ //default should be: CU_GRAPHICS_REGISTER_FLAGS_NONE
    ...
    if( m_cudaTextureResource != nullptr && usageflags!=m_cudaUsageFlags){
