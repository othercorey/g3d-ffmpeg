/**
  \file G3D-gfx.lib/source/Shader.cpp
...
void Shader::bindUniformArg(const Args::Arg& arg, const ShaderProgram::UniformDeclaration& decl, int& maxModifiedTextureUnit) {
...
#if 1       //Non bindless
    {
            ...
            const bool layered    =  (decl.type == GL_IMAGE_3D || decl.type == GL_IMAGE_CUBE) ? GL_TRUE : GL_FALSE;
            const GLint layer     = 0;
            const GLint mipLevel  = arg.value[0].ui;
            const GLenum access   = arg.value[1].ui;
            const GLenum glformat = arg.texture->format()->openGLShaderImageFormat();

            // See https://www.khronos.org/registry/OpenGL-Refpages/es3.1/html/glTexStorage2D.xhtml
            glBindImageTexture(decl.glUnit, arg.texture->openGLID(), mipLevel, layered, layer, access, glformat);
            // The above call will fail with GL_INVALID_OPERATION under GL ES if the texture does not have immutable storage.
    ...
    case GL_FLOAT_MAT2x3:
        {
            ...
            glUniformMatrix2x3fv(location, 1, GL_FALSE, m);
        }
        break;
...
#ifndef G3D_WINDOWS
    // We don't use G3DString on OS X, unfortunately
    bool beginsWith_g3d_(const String& s) {
        return beginsWith(s, SYMBOL_g3d_);
    }
#else
    __forceinline bool beginsWith_g3d_(const G3DString<64>& s) {
    ...
    for (Args::GPUAttributeTable::Iterator i = t.begin(); i != t.end(); ++i){
        ...
        if (beginsWith_g3d_(name)) { 
            ...
            if (notNull(declPtr)) {
                ...
                alwaysAssertM(false, format("Tried to bind attribute %s on the C++ side that was not used in the shader.\n", name.c_str()));
            }            
        } 
    }
...
shared_ptr<Shader::ShaderProgram> Shader::compileAndBind(const Args& args, RenderDevice* rd, int& maxModifiedTextureUnit) {
    ...
    // Not const because it may be overwritten below by handleRecoverableError
...
shared_ptr<Shader> Shader::getShaderFromPattern(const String& pattern) {
    ...
    if (absolutePattern.empty()) {
        const size_t prefixLoc = pattern.find("_");
...
#   define TEXARG(nameString, val)\
    ...
    if (p->containsNonDummyUniform("g3d_ProjInfo")) {
        const Matrix4& P = renderDevice->projectionMatrix();
        const ProjInfo projInfo(float(-2.0 / (renderDevice->width() * P[0][0])), 
            float(-2.0 / (renderDevice->height() * P[1][1])),
            float((1.0 - (double)P[0][2]) / P[0][0]), 
            float((1.0 - (double)P[1][2]) / P[1][1]));
        ARG("g3d_ProjInfo", projInfo);
    }

    if (p->containsNonDummyUniform("g3d_ClipInfo")) {
        const Projection projection(renderDevice->projectionMatrix(), renderDevice->viewport().wh());
        ARG("g3d_ClipInfo", projection.reconstructFromDepthClipInfo());
    }
    ...
    ARG("g3d_FramebufferSize", renderDevice->framebuffer()->vector2Bounds());
...
void Shader::loadAndPreprocess(const Args& args, Array<PreprocessedShaderSource>& preprocessedSource) {
    String loadMessages;
    bool ok = true;
    ...
    for (int s = 0; s < STAGE_COUNT; ++s) {
        ...
        debugAssertGLOk();
        if (! code.empty()) {
            ok = g3dLoadTimePreprocessor(dir, pSource, loadMessages, toGLEnum(ShaderStage(s)), args) && ok;
            if (! ok) {
                break;
            }
        }
        preprocessedSource.append(pSource);
    }

    if (ok &&
        preprocessedSource[VERTEX].preprocessedCode.empty() &&
        ! preprocessedSource[PIXEL].preprocessedCode.empty()) {
        ...
        preprocessedSource[VERTEX].filename         = defaultVertexShaderFile;
        preprocessedSource[VERTEX].preprocessedCode = defaultVertexShaderCode;
        preprocessedSource[VERTEX].versionString    = GLCaps::api() == OSWindow::Settings::API_OPENGL_ES ? "#version 310 es\n" : "#version 410\n";

        String ignore;
        processExtensions(ignore, preprocessedSource[VERTEX].extensionsString);
    }
...
void Shader::ShaderProgram::printUniformDeclarations() const {
    for (const UniformDeclarationTable::Entry& entry : uniformDeclarationTable) {
        const char* maybeComment = entry.value.dummy ? "// " : "";
        debugPrintf("%slayout(location=%d) uniform %s %s;\n", maybeComment, entry.value.location, GLenumToString(entry.value.type), entry.key.c_str());
    }
}
...
void Shader::handleRecoverableError(RecoverableErrorType eType, const Args& args, const String& messages, shared_ptr<ShaderProgram>& program){
    if (s_failureBehavior == PROMPT) {
        ...
        if (notNull(program)) {
            debugPrintf("-----------------------------------------------------------------\n");
            debugPrintf("Uniform declarations:\n");
            program->printUniformDeclarations();
            debugPrintf("\n");
        } else {
            debugPrintf("-----------------------------------------------------------------\n");
            debugPrintf("Preamble and macro arguments:\n");
            debugPrintf("%s\n\n", args.preambleAndMacroString().c_str());
            debugPrintf("Uniform arguments:\n");

            // Print alphabetically
            Array<String> keyArray;
            args.uniformTable().getKeys(keyArray);
            keyArray.sort();
            for (const String& key : keyArray) {   
                debugPrintf("%-20s %s\n", GLenumToString(args.uniformTable()[key].type), key.c_str());
            }
            debugPrintf("\n");
        }
        ...
        // Copy the result into a std::string temporarily in case
        // G3D::String is the cause of the problem.
        ...
        while (split != std::string::npos) {
            ...
            const String& lowerCaseError = toLower(singleError.c_str());
            // The error is considered valid (not just a warning) if it does not both the words "extension" and "warning"
        ...
        const int userAction = prompt("Shader Compilation Failed",
            output.c_str(),
            (const char**)options,
            3,
            true);
...
shared_ptr<Shader::ShaderProgram> Shader::shaderProgram(const Args& args, String& messages) {
    ...
    debugAssertGLOk();
    // m_preprocessedSource is an array of the source code for each shader stage (some of which are empty).
    // Each shader stage will have preambleAndMacroString prepended to it and then be compiled by
    // the following line.
    const shared_ptr<ShaderProgram>& s = ShaderProgram::create(preprocessedSource, preambleAndMacroString, args, m_indexToFilenameTable);
    debugAssertGLOk();
...
shared_ptr<Shader::ShaderProgram> Shader::retry(const Args& args){
    reload(); 
    String messages;
    // Not const because passed to handleRecoverableError
    shared_ptr<ShaderProgram> program = shaderProgram(args, messages);
...
shared_ptr<Shader> Shader::create(const Specification& s){
    const shared_ptr<Shader>& shader = createShared<Shader>(s);
    // Defer actual loading until the first call site because
    // we need macro arguments to process some #include values
