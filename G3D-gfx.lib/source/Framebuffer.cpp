/**
  \file G3D-gfx.lib/source/Framebuffer.cpp
...
Framebuffer::Framebuffer(
    ...
    m_invertY(framebufferID == GL_ZERO),
    m_window(nullptr) {
...
shared_ptr<Framebuffer> Framebuffer::create(const shared_ptr<Texture>& t0, const shared_ptr<Texture>& t1, const shared_ptr<Texture>& t2, const shared_ptr<Texture>& t3) {
...
void Framebuffer::set(AttachmentPoint ap, const shared_ptr<Texture>& texture, CubeFace face, int mipLevel, int layer) {
    alwaysAssertM(m_framebufferID != GL_ZERO, "Cannot bind attachments to a hardware framebuffer");
    alwaysAssertM(! m_noAttachment, "Can't set attachment to a Frambuffer in no-attachment mode");
    ...
    const shared_ptr<Attachment>& a = get(ap);
    if (isNull(a) || ! (a->equals(texture, face, mipLevel, layer))) {
        // This is a change
        set(Attachment::create(ap, texture, face, mipLevel, layer));
    }
}
...
/*
    if (m_currentOutOfSync) {
//        const_cast<Framebuffer*>(this)->sync();
        const_cast<Framebuffer*>(this)->bind();
    }
    if (notNull(dst)) {
        //dst->sync();
        dst->bind(false, Framebuffer::MODE_DRAW);
    }

*/
    // Make sure all attachments are properly bound before blitting
    rd->syncDrawBuffer(true);
    rd->syncReadBuffer(true);
    debugAssertGLOk();
...
void Framebuffer::Attachment::attach() const {
    ...
    if (m_type == TEXTURE) {
        ...
        if (isCubeMap) {
            glFramebufferTexture2D(GL_FRAMEBUFFER, GLenum(m_point), GL_TEXTURE_CUBE_MAP_POSITIVE_X + (int)m_cubeFace, m_texture->openGLID(), m_mipLevel);
        } else if ((is2DArray || is3D) && (m_layer >= 0)) {
            glFramebufferTextureLayer(GL_FRAMEBUFFER, GLenum(m_point), m_texture->openGLID(), m_mipLevel, m_layer);
        } else if (isCubeMapArray && (m_layer >= 0)) {
            glFramebufferTextureLayer(GL_FRAMEBUFFER, GLenum(m_point), m_texture->openGLID(), m_mipLevel, m_layer * 6 + m_cubeFace);
        } else {
            glFramebufferTexture(GL_FRAMEBUFFER, GLenum(m_point), m_texture->openGLID(), m_mipLevel);
        }
        debugAssertGLOk();
    }
}
