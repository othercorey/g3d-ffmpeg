/**
  \file G3D-gfx.lib/include/G3D-gfx/UniformTable.h
...
/** \brief Uniform and macro arguments for a G3D::Shader.
    ...
    \sa Args
*/
class UniformTable : public ReferenceCountedObject {
public:
    friend class Shader;
...
public:
    ...
    */
    ...
    /** Returns the empty string if the macro is not bound. \see getMacro */
    String macro(const String& name) const {
        String s;
        if (getMacro(name, s)) { return s; } else { return ""; }
    }
