/**
  \file G3D-gfx.lib/include/G3D-gfx/Args.h
...
/** \brief Arguments to use when running a Shader. 
 ...
 G3D also supports a special way to include macros with newlines and preprocessor commands in them. In your shader,
 <code>#includestring macroarg</code> inserts the value of the macroarg (it does <b>not</b> include a file named
 by the macroarg). It is an error to insert such macros by name only.
...
private:
    ...
    Vector3int32        m_computeGroupSize;
...
public:
    ...
    /** @deprecated */
    [[deprecated]] void clearAttributeArgs() {
    ...
    */
    ...
    /** If you change the number of instances in order to produce multiple
      ...
      \sa G3D::ArticulatedModel::Pose::Pose, G3D::ArticulatedModel::Pose::numInstances, G3D::UniversalSurface::create
     */
