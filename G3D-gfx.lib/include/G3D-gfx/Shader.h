/**
  \file G3D-gfx.lib/include/G3D-gfx/Shader.h
...
/**
  ...
  <h2>Preprocessor</h2>
   ...
   `#include MACRONAME` Loads the file specified by the <b>fully qualified</b> macro argument's value.
   The macro must be defined as a shader argument, not as a \#define.

   Every included file effectively has `#pragma once`, which is done by the preprocessor to prevent
   infinitely recursive includes.
   ...
   `#eval(macroname)` is identical to just using `macroname` if the macro contains no newlines and is a
   shader macro argument. It cannot be applied to macros that are defined in the shader using \#define.

   For shader macro arguments that contain newlines, \#eval inserts their values with newlines and preserving surrounding
   line numbers for error reporting. Those macros are defined as blank for the regular preprocessor. This allows 
   idioms such as:
...
protected:
    ...
    public:
        ...
        public:
            ...
            /** The number of elements in this variable. For non-arrays, this is 1. For now, we
                store all array elements separately, so this is always 1. Left for future
                use. */
            GLuint      elementNum;
            
            /** The index of this element in its array. For non-array elements, -1. */
            GLuint      index;

            /** The texture, image, or shader buffer unit this uniform is bound to */
            GLint       glUnit;

            /** If true, this uniform is not used by the shader program, so we never try to
                bind a value to it */
            bool        dummy;

            /** Shorthand to avoid having to set these fields one-by-one. Not in a constructor
                since UniformDeclaration is a G3D::Table key in its main role and thus gets
    ...
    */
    ...
    /** Finds a shader program in the cache that matches the supplied
        ...
        nullptr and fills in messages with a report of what went wrong.
    ...
    */
    ...
    /** Array of all shaders ever created.  nullptr elements are flushed during reloadAll() */
...
public:
    ...
    */
    ...
    @return True if we found and removed a version line from the code.
        ...
        \deprecated
    */
    [[deprecated]] static void processExtensions(String& code, String& extensionLines);
...
G3D::System::findDataFile. If this fails, the pattern is searched for an underscore. If there is an underscore, everything
...
Note that the underlying G3D::Shader class and G3D::RenderDevice::apply call are available for usage scenarios that do not
...
#define LAUNCH_SHADER_WITH_HINT(pattern, args, hint) {\
    ...
    const bool __shaderTimingsEnabled = G3D::Profiler::LAUNCH_SHADER_timingEnabled();\
    if (__shaderTimingsEnabled) {\
        G3D::Profiler::beginEvent(__theShader->name(), __FILE__, __LINE__, hint);\
    }\
...
#define LAUNCH_SHADER_PTR_WITH_HINT(shader, args, hint) { \
    const bool __shaderTimingsEnabled = G3D::Profiler::LAUNCH_SHADER_timingEnabled();\
    if (__shaderTimingsEnabled) {\
        G3D::Profiler::beginEvent(shader->name(), __FILE__, __LINE__, hint);\
    }\
