/**
  \file G3D-gfx.lib/include/G3D-gfx/GKey.h
...
/** Analog controller axes. Each is 2D, although the y component is not used for all.
    \sa G3D::OSWindow, G3D::GameController, G3D::GKey, G3D::UserInput
 */
G3D_DECLARE_ENUM_CLASS(JoystickIndex,
    /** Left thumb stick */
    LEFT,
    ...
    /** Grip/shoulder/bumper button. Only the x axis is used */
    LEFT_SHOULDER,
...
/** 
   ...
   characters (use <b>lower-case letters</b> for alphabetic keys).

   Not all GKeys are generated as explicit events.  E.g., the MOUSE
   and CONTROLLER are solely for use with UserInput.
   
    \sa G3D::OSWindow, G3D::GameController, G3D::JoystickIndex, G3D::UserInput
*/ 
class GKey {
public:
    /** Don't use this enum directly; use GKey instances instead. */
    enum Value {
        UNKNOWN      = 0,
        FIRST        = 0,
        BACKSPACE    = 8,
        ...
        */
        ...
        /** Xbox-style dual stick controller */

        CONTROLLER_A,  // The order of the next 15 buttons is assumed by UserInput.cpp
        CONTROLLER_B,
        CONTROLLER_X,
        CONTROLLER_Y,
        /** Also called the shoulder button or grip button in VR */
        ...
        /** The logo/power/home button, e.g., the one with the Xbox
            Guide logo on an Xbox controller*/
        CONTROLLER_GUIDE,
        ...
        CONTROLLER_RESERVED_1,
        CONTROLLER_RESERVED_2,
        CONTROLLER_RESERVED_3,
        CONTROLLER_RESERVED_4,
        CONTROLLER_LAST = CONTROLLER_RESERVED_4,
...
public:
    ...
    static GKey fromString(const String& _s);
    ...
    GKey(const Value v) : value(v) {}

    /** Allow construction, but not implicit casts */
    explicit GKey(int v) : value((Value)v) {
        debugAssertM(v < 'A' || v > 'Z', "Use lower-case letters for keyboard letters. Capital leters are never returned by event handlers");
    }
    ...
    bool operator== (const GKey::Value other) const {
        return value == other;
    }
    ...
    GKey operator+ (const int x) const {
        return GKey((int)value + x);
    }
